...
#                                z.(lagbwprob) + z.(lagcumwdelta))^2 + (1|word)
             ...
             bform <- clfdur ~ z.(sentpos) + z.(wlen) + z.(prevwasfix) +
                                z.(cumwdelta) + (1|word)
...
if (OPTIM == "bobyqa"){
    ...
    baseinteg <- lmer(update.formula(bform,.~.+(1+z.(sentpos)+z.(wlen)+z.(prevwasfix)+z.(fwprob)+z.(cumwdelta) + z.(cumfwprob) |subject)), test, REML=F, control = lmerControl(optimizer="bobyqa",optCtrl=list(maxfun=50000)))
    print("Regressing for fwprob")
    write("Regressing for fwprob",stderr())
    fwinteg <- lmer(update.formula(bform,.~.+z.(fwprob) + (1+z.(sentpos)+z.(wlen)+z.(prevwasfix)+z.(fwprob)+z.(cumwdelta) +z.(cumfwprob) |subject)), test, REML=F, control = lmerControl(optimizer="bobyqa",optCtrl=list(maxfun=50000)))
    print("Regressing for cumfwprob")
    write("Regressing for cumfwprob",stderr())
    cumfwinteg <- lmer(update.formula(bform,.~.+z.(cumfwprob) + (1+z.(sentpos)+z.(wlen)+z.(prevwasfix)+z.(fwprob)+z.(cumwdelta) + z.(cumfwprob)|subject)), test, REML=F, control = lmerControl(optimizer="bobyqa",optCtrl=list(maxfun=50000)))
    print("Regressing Both")
    write("Regressing Both",stderr())
    bothinteg <- lmer(update.formula(bform,.~.+z.(fwprob) + z.(cumfwprob) + (1+z.(sentpos)+z.(wlen)+z.(prevwasfix)+z.(fwprob)+z.(cumwdelta) + z.(cumfwprob) |subject)), test, REML=F, control = lmerControl(optimizer="bobyqa",optCtrl=list(maxfun=50000)))
}
...
if (OPTIM == "nlminb"){
     print("Regressing Base")
     write("Regressing Base",stderr())
     baseinteg <- lmer(update.formula(bform,.~.+(1+z.(sentpos)+z.(wlen)+z.(prevwasfix)+z.(fwprob)+z.(cumwdelta) + z.(cumfwprob) |subject)), test, REML=F, control = lmerControl(optimizer="optimx",optCtrl=list(method=c("nlminb"),maxit=50000)))
     print("Regressing fwprob")
     write("Regressing fwprob",stderr())
     fwinteg <- lmer(update.formula(bform,.~.+z.(fwprob) + (1+z.(sentpos)+z.(wlen)+z.(prevwasfix)+z.(fwprob)+z.(cumwdelta) + z.(cumfwprob) |subject)), test, REML=F, control = lmerControl(optimizer="optimx",optCtrl=list(method=c("nlminb"),maxit=50000)))
     print("Regressing cumfwprob")
     write("Regressing cumfwprob",stderr())
     cumfwinteg <- lmer(update.formula(bform,.~.+z.(cumfwprob) + (1+z.(sentpos)+z.(wlen)+z.(prevwasfix)+z.(fwprob)+z.(cumwdelta) + z.(cumfwprob) |subject)), test, REML=F, control = lmerControl(optimizer="optimx",optCtrl=list(method=c("nlminb"),maxit=50000)))
     print("Regressing Both")
     write("Regressing Both",stderr())
     bothinteg <- lmer(update.formula(bform,.~.+z.(fwprob) + z.(cumfwprob) + (1+z.(sentpos)+z.(wlen)+z.(prevwasfix)+z.(fwprob)+z.(cumwdelta) + z.(cumfwprob) |subject)), test, REML=F, control = lmerControl(optimizer="optimx",optCtrl=list(method=c("nlminb"),maxit=50000)))
  }
...
#  baseinteg <- lmer(update.formula(bform,.~.+z.(cuminteg)+(1|subject)), test, REML=F)
    ...
    print("cumfw Summary")
    print(summary(cumfwinteg))
    print("cumfw Log Likelihood")
    print(logLik(cumfwinteg))
    relgrad <- with(cumfwinteg@optinfo$derivs,solve(Hessian,gradient))
    print("Relative Gradient (<0.001?)") #check for convergence even if warned that convergence failed
    print(max(abs(relgrad)))
    print("cumfw AIC")
    print(AIC(logLik(cumfwinteg)))

    print("fw Summary")
    print(summary(fwinteg))
    print("fw Log Likelihood")
    print(logLik(fwinteg))
    relgrad <- with(fwinteg@optinfo$derivs,solve(Hessian,gradient))
    print("Relative Gradient (<0.001?)") #check for convergence even if warned that convergence failed
    print(max(abs(relgrad)))
    print("fw AIC")
    print(AIC(logLik(fwinteg)))
    ...
    print("Base vs cumfw")
    print(anova(baseinteg,cumfwinteg))
    print("Base vs fw")
    print(anova(baseinteg,fwinteg))
    print("cumfw vs Both")
    print(anova(cumfwinteg,bothinteg))
    print("fw vs Both")
    print(anova(fwinteg,bothinteg))
