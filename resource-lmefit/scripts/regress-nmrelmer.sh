...
base_filename="$dname$(basename $prdmeasures _part.prdmeasures)_$(basename $bform .nmrelmerform)"
if [ -n "$preds_STR" ]; then
...
outfile+="_nmrelmer.fitmodel.rdata";
...
../resource-lmefit/scripts/evmeasures2lmefit.r <(cat $tblfile) $outfile -b $bform --noMainRandomEffect $preds_add_STR $preds_ablate_STR -c $corpusname -e
