...
#base_filename="$dname$(basename $prdmeasures _part.prdmeasures)_$(basename $bform .gamform)_$preds_STR"
base_filename="$dname$(basename $prdmeasures _part.prdmeasures)_$(basename $bform .gamform)"
if [ -n "$preds_STR" ]; then
...
../resource-gamfit/scripts/evmeasures2gamfit.r <(cat $tblfile) $outfile -b $bform $preds_add_STR $preds_ablate_STR -c $corpusname -e
