...
processLMEArgs <- function() {
    ...
    opt_list <- list(
        ...
        make_option(c('-m', '--fitmode'), type='character', default='gam', help='Fit mode. Currently supports "lme" (linear mixed effects), "bme" (Bayesian mixed effects), and "lm" (simple linear regression, which discards all random terms). Defaults to "lme".'),
...
# s(subject, surprisal, bs = "fs", m = 1) +
# te(freq, length)

# s(subject, bs = 're') is equivalent to (1 | subject)
...
formlist2form <- function(formList, interact, include_random=TRUE) {
    ...
    if (include_random) {
        formStr <- paste0(formList$dep, ' ~ ', formList$fixed, ' + ', formList$by_subject)
    } else {
...
regressGeneralizedAdditiveModel <- function(dataset, form) {
    library(mgcv)
    # library(optimx)
    # bobyqa <- lmerControl(optimizer="bobyqa",optCtrl=list(maxfun=50000))
    # nlminb <- lmerControl(optimizer="optimx",optCtrl=list(method=c("nlminb"),maxit=50000))
    ...
    smartPrint('-----------------------------')
    smartPrint('Fitting generalized additive model')
    smartPrint(paste(' ', date()))
    m <- gam(form, data=dataset)
    smartPrint('-----------------------------')
    smartPrint('SUMMARY:')
    printGAMSummary(m)
    smartPrint('logLik:')
...
printGAMSummary <- function(m) {
    cat(paste0('GAM Summary:\n'))
    print(summary(m))
}
...
fitModel <- function(dataset, output, bformfile, fitmode='gam',
    ...
    if (fitmode=='bme') {
        ...
        outputModel <- regressGeneralizedAdditiveModel(dataset, bform)
    } else {
