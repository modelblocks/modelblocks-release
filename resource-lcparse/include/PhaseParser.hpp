...
static struct option long_options[] = {
  ...
  {"beam_verbose", no_argument, 0, 'B'},
  {"entropy_verbose", no_argument, 0, 'E'},
...
bool               VERBOSE        = false;
bool               BEAM_VERBOSE        = false;
bool               ENTROPY_VERBOSE        = false;
int                VERY_VERBOSE   = 0;
...
class PhaseParser : public ModelReader<YM> {
 ...
 private:
  ...
  int                                 t;
  SimpleMap<X,double> vocabprobs;
  SimpleMap<X,double> prevvocabprobs;

  double prevembeddep;
  double vocabentropy;
  double prevvocabentropy;
  double preventropy;
  ...
  void update ( const X& x, const X& prevx ) {
    // Init new beam...
    ...
    for (auto &xmodelitem : ModelReader<YM>::getModel().getXModel()) {
      //xmodelitem.first is the POS, xmodelitem.second is the <X, LogProb> map
      for (auto &lexeme : xmodelitem.second) {
        //lexeme.first is the lexical item, lexeme.second is the LogProb
        vocabprobs.set(lexeme.first) = 0.0;
        prevvocabprobs.set(lexeme.first) = 0.0;
      }
    }
    ...
    if ( REPORT_COMPLEX ) {
      if ( BEAM_VERBOSE ) {
        /* write beam probs */
        double thisbeamprob = 0.0;
        double thislexsurp = 0.0;
        cout << "[";
        for ( unsigned int ix=0; ix<beams[t].size(); ix++ ) {
          thisbeamprob = beams[t][ix].getProb().toDouble();
          thislexsurp = ModelReader<YM>::getModel().getXModel().getProb(beams[t][ix].getY().getP().getC(),x).toDouble();
          cout << thisbeamprob << "," << thislexsurp << "," << beams[t].getBack(ix) << "=";
        }
        cout << "]\n";
      }
      ...
      if ( beams[t].size() != 0 ) { //Since beams is an array of hashes, accessing [0] creates it, so make sure it exists
        ...
        double myprevmaxNorm = 0.0;
        if ( t > 1 ) {
          LogProb myprevmax = beams[t-1][0].getProb(); //Scale everything by the largest thing in the beam to obtain same relative metrics without underflow
          for ( unsigned int ix=0; ix<beams[t-1].size(); ix++ ) {
            myprevmaxNorm += (beams[t-1][ix].getProb()/myprevmax).toDouble(); //Create a scaled normalization constant to prevent entred underflow
          }
        }
        else { myprevmaxNorm = 1.0; }
        ...
        if ( ENTROPY_VERBOSE ) {          
          LogProb thislexprob = 0.0;
          LogProb prelexprob = 0.0;
          for ( unsigned int ix=0; ix<beams[t].size(); ix++ ) {
            //for each element in the beam, determine the prob of each lexical item
            thislexprob = ModelReader<YM>::getModel().getXModel().getProb(beams[t][ix].getY().getP().getC(),x);
            prelexprob = (beams[t][ix].getProb()) - thislexprob;
            for ( auto &vocabitem : vocabprobs ) {
              //vocabitem.first is the lexical item, vocabitem.second is the prob
              vocabprobs.set(vocabitem.first) = vocabitem.second/mymaxNorm + ModelReader<YM>::getModel().getXModel().getProb(beams[t][ix].getY().getP().getC(),vocabitem.first).toDouble()/mymaxNorm * prelexprob.toDouble()/mymaxNorm;
              //cout << "\n vocabprobs item " << vocabprobs.get(vocabitem.first);
            }
          }
          if ( t > 1 ) {
            LogProb thislexprob = 0.0;
            LogProb prelexprob = 0.0;
            for ( unsigned int ix=0; ix<beams[t-1].size(); ix++ ) {
              thislexprob = ModelReader<YM>::getModel().getXModel().getProb(beams[t-1][ix].getY().getP().getC(),prevx);
              prelexprob = (beams[t-1][ix].getProb()) - thislexprob;
              for ( auto &vocabitem : prevvocabprobs ) {
                //vocabitem.first is the lexical item, vocabitem.second is the prob
                prevvocabprobs.set(vocabitem.first) = vocabitem.second/myprevmaxNorm + ModelReader<YM>::getModel().getXModel().getProb(beams[t-1][ix].getY().getP().getC(),vocabitem.first).toDouble()/myprevmaxNorm * prelexprob.toDouble()/myprevmaxNorm;
              }
            }
          }
          prevvocabentropy = 0.0;
          vocabentropy = 0.0;
          //compute entropy of the vocabulary
          for ( auto &vocabitem : vocabprobs ) {
            //vocabitem.first is the lexical item, vocabitem.second is the prob
            if ( vocabitem.second != 0 ) {
              // somehow, there are bogus vocab items (-, --)
              vocabentropy -= vocabitem.second * log2(vocabitem.second);
            }
            if ( prevvocabprobs.get(vocabitem.first) != 0 ) {
              prevvocabentropy -= prevvocabprobs.get(vocabitem.first) * log2(prevvocabprobs.get(vocabitem.first));
            }
          }
        }
...
/**/cerr<<x<<" ("<<beams[t].size()<<") ";                                    // %2
 ...
 public:
  ...
  PhaseParser ( int nArgs, char* argv[], const Y& Y_INIT ) : ModelReader<YM>(nArgs,argv) {
    ...
    // Check flags...
    // char opt;
    for( int opt, option_index = 0; (opt = getopt_long(nArgs, argv, "vV:cBEb:", long_options, &option_index)) != -1; ) {
      switch(opt) {
      ...
      case 'B': BEAM_VERBOSE = true; break;
      case 'E': ENTROPY_VERBOSE = true; break;
