...
rois = c('LPostTemp','LAntTemp','LAngG','LIFG','LMFG','LIFGorb')
...
responseVars = paste0('bold', rois, '.')
for (s in subjs) {
dfw[paste0('bold', 'Mean.', s)] = rowMeans(dfw[paste0('bold', rois, '.', s)])
}
...
for (s in subjs) {
    print(cor(dfw[paste0('bold',rois,'.',s)]))
    cat('\n')
}
...
for (i in 1:length(rois)) {
    x = as.numeric(cor(df[paste0('bold', rois[i])], rowMeans(df[paste0('bold', rois[-i])])))
    cat(paste0('bold', rois[i], ': ', x, '\n'))
}
cat('\n')
