...
class trip {
 public:
  trip ( ) { }
  trip ( const A& a, const B& b, const C& c ) : first(a), second(b), third(c) { }
  A first;
  B second;
  C third;
...
class quad {
 public:
  quad ( ) { }
  quad ( const A& a, const B& b, const C& c, const D& d ) : first(a), second(b), third(c), fourth(d) { }
  A first;
  B second;
  C third;
  D fourth;
...
class penta {
 public:
  penta ( ) { }
  penta ( const A& a, const B& b, const C& c, const D& d, const E& e) : first(a), second(b), third(c), fourth(d), fifth(e) { }
  A first;
  B second;
  C third;
  D fourth;
...
class Joint5DRV {
 ...
 public:
  ...
  // Constructor / destructor methods...
  Joint5DRV ( )                                          { }
  Joint5DRV ( const V1& v1, const V2& v2, const V3& v3, const V4& v4, const V5& v5 ) { first=v1; second=v2; third=v3; fourth=v4;fifth=v5;}
  // Extraction methods...
  size_t getHashKey ( ) const { size_t k=rotLeft(first.getHashKey(),3); k^=second.getHashKey(); k=rotLeft(k,3); k^=third.getHashKey();k^=fourth.getHashKey();k^=fifth.getHashKey();
                                /*fprintf(stderr,"  (%d) %d ^& %d = %d\n",sizeof(*this),x1.getHashKey(),x2.getHashKey(),k);*/ return k; }
//  bool      operator< ( const Joint2DRV<V1,V2>& j )  const { return ( (first<j.first) ||
//                                                                      (first==j.first && second<j.second) ); }
  bool      operator== ( const Joint5DRV<V1,V2,V3,V4,V5>& j ) const { return ( first==j.first && second==j.second && third==j.third && fourth==j.fourth && fifth==j.fifth ); }
  bool      operator!= ( const Joint5DRV<V1,V2,V3,V4,V5>& j ) const { return ( !(first==j.first && second==j.second && third==j.third && fourth==j.fourth && fifth == j.fifth) ); }
};

////////////////////////////////////////////////////////////
template<char* SD1,class V1,char* SD2,class V2,char* SD3,class V3,char* SD4,class V4,char* SD5,class V5, char* SD6>
class DelimitedJoint5DRV : public Joint5DRV<V1,V2,V3,V4,V5> {

 public:

  static const int NUM_VARS = V1::NUM_VARS + V2::NUM_VARS + V3::NUM_VARS+ V4::NUM_VARS+V5::NUM_VARS;

  ////////////////////
  template<class P>
  class ArrayIterator : public penta<typename V1::template ArrayIterator<P>, typename V2::template ArrayIterator<P>, typename V3::template ArrayIterator<P> , typename V4::template ArrayIterator<P> ,typename V5::template ArrayIterator<P> > {
   public:
    // static const int NUM_ITERS = (typename V1::template ArrayIterator<P>)::NUM_ITERS + (typename V2::template ArrayIterator<P>)::NUM_ITERS;
    static const int NUM_ITERS = NUM_VARS;
    friend ostream& operator<< ( ostream& os, const ArrayIterator<P>& rv ) {
      return os<<SD1<<rv.first<<SD2<<rv.second<<SD3<<rv.third<<SD4<<rv.fourth<<SD5<< rv.fifth<<SD6; }
  };

  // Constructor / destructor methods...
  DelimitedJoint5DRV ( )                                                                       : Joint5DRV<V1,V2,V3,V4,V5>()               { }
  DelimitedJoint5DRV ( const V1& v1, const V2& v2, const V3& v3, const V4& v4, const V5& v5 )  : Joint5DRV<V1,V2,V3,V4,V5>(v1,v2,v3,v4,v5) { }
  DelimitedJoint5DRV ( char* ps )                                                              : Joint5DRV<V1,V2,V3,V4,V5>()               { ps>>*this>>"\0"; }
  DelimitedJoint5DRV ( const char* ps )                                                        : Joint5DRV<V1,V2,V3,V4,V5>()               { strdup(ps)>>*this>>"\0"; }

  // Specification methods...
  template<class P>
  DelimitedJoint5DRV<SD1,V1,SD2,V2,SD3,V3,SD4,V4,SD5,V5,SD6>& setVal ( const ArrayIterator<P>& it ) {
    Joint5DRV<V1,V2,V3,V4,V5>::first.setVal(it.first);
    Joint5DRV<V1,V2,V3,V4,V5>::second.setVal(it.second);
    Joint5DRV<V1,V2,V3,V4,V5>::third.setVal(it.third);
    Joint5DRV<V1,V2,V3,V4,V5>::fourth.setVal(it.fourth);
    Joint5DRV<V1,V2,V3,V4,V5>::fifth.setVal(it.fifth);
    return *this;
    }

  // Extraction methods...
  bool operator==(const DelimitedJoint5DRV<SD1,V1,SD2,V2,SD3,V3,SD4,V4,SD5,V5,SD6>& vvvv) const { return Joint5DRV<V1,V2,V3,V4,V5>::operator==(vvvv); }
  bool operator< (const DelimitedJoint5DRV<SD1,V1,SD2,V2,SD3,V3,SD4,V4,SD5,V5,SD6>& vvvv) const { return Joint5DRV<V1,V2,V3,V4,V5>::operator< (vvvv); }

  // Input / output methods...
  friend ostream& operator<< ( ostream& os, const DelimitedJoint5DRV<SD1,V1,SD2,V2,SD3,V3,SD4,V4,SD5,V5,SD6>& rv ) { return  os<<SD1<<rv.first<<SD2<<rv.second<<SD3<<rv.third<<SD4<<rv.fourth<<SD5<<rv.fifth<<SD6; }
  friend String&  operator<< ( String& str, const DelimitedJoint5DRV<SD1,V1,SD2,V2,SD3,V3,SD4,V4,SD5,V5,SD6>& rv ) { return str<<SD1<<rv.first<<SD2<<rv.second<<SD3<<rv.third<<SD4<<rv.fourth<<SD5<<rv.fifth<<SD6; }
  friend pair<StringInput,DelimitedJoint5DRV<SD1,V1,SD2,V2,SD3,V3,SD4,V4,SD5,V5,SD6>*> operator>> ( StringInput ps, DelimitedJoint5DRV<SD1,V1,SD2,V2,SD3,V3,SD4,V4,SD5,V5,SD6>& rv ) {
    return pair<StringInput,DelimitedJoint5DRV<SD1,V1,SD2,V2,SD3,V3,SD4,V4,SD5,V5,SD6>*>(ps,&rv); }
  friend StringInput    operator>> ( pair<StringInput,DelimitedJoint5DRV<SD1,V1,SD2,V2,SD3,V3,SD4,V4,SD5,V5,SD6>*> delimbuff, const char* psDlm ) {
    if (StringInput(NULL)==delimbuff.first) return delimbuff.first;
    return ( (SD6[0]=='\0') ? delimbuff.first>>SD1>>delimbuff.second->first>>SD2>>delimbuff.second->second>>SD3>>delimbuff.second->third>>SD4>>delimbuff.second->fourth>>SD5>>delimbuff.second->fifth>>psDlm
                            : delimbuff.first>>SD1>>delimbuff.second->first>>SD2>>delimbuff.second->second>>SD3>>delimbuff.second->third>>SD4>>delimbuff.second->fourth>>SD5>>delimbuff.second->fifth>>SD6>>psDlm );
  }
};
...
class DelimitedJointArrayRV : public JointArrayRV<I,T> {
 public:
  ...
  friend StringInput operator>> ( pair<StringInput,DelimitedJointArrayRV<I,SD,T>*> delimbuff, const char* psDlm ) {
    ...
    StringInput psIn = delimbuff.first;
    for(int i=0;i<I;i++)
      psIn = psIn>>delimbuff.second->set(i)>>((i<I-1)?SD:psDlm);
