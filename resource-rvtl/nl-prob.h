...
class LogProb : public Id<int> {
 public:
  ...
  LogProb ( int i )    { set(i); }
  LogProb ( double d ) { set(int(FIX_PT_FACTOR*log(d))); }
  LogProb ( Prob   d ) { set(int(FIX_PT_FACTOR*log(d))); }
  // Specification methods...
  LogProb& operator+= ( const LogProb i ) { assert(false); return *this; } // no support for addition in log mode!
  ...
  LogProb& operator-= ( const LogProb i ) { assert(false); return *this; } // no support for subtraction in log mode!
  LogProb& operator/= ( const LogProb i )
    {   if((toInt() == MIN_LOG_PROB) || (i.toInt() == MIN_LOG_PROB)){
            set(MIN_LOG_PROB);
        }else{
  ...
  LogProb operator- ( const LogProb i ) const { assert(false); return *this; }   // no support for subtraction in log mode!
  ...
  Prob   toProb()   const { return exp(double(toInt())/FIX_PT_FACTOR);  }
  double toDouble() const { return toProb().toDouble(); }
//  operator double() const { return exp(toInt()/FIX_PT_FACTOR); }
