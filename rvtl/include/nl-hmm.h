...
class HMM {
 ...
 public:
  ...
  void updateRankedCFP ( const typename MX::RandVarType&, bool ) ;
...
template <class MY, class MX, class S, class B>
void HMM<MY,MX,S,B>::updateRankedCFP ( const typename MX::RandVarType& x, bool b1 ) {

  // initialize set of partial-path hypotheses (priority queue) with first (most probable) element of trellis at prev time step

  // iterate over set of partial-path hypotheses

    // obtain the leading hypothesis

    // take a forward step, and record new result in priority queue

    // take a side step, and record new result in priority queue
