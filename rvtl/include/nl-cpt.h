...
public:
	...
	friend String& operator<<(String& str, const Unit& u) {
		return str;
	}
	friend IStream operator>>(pair<IStream, Unit*> si_m, const char* psD) {
		return si_m.first;
	}

	// OBSOLETE!
	friend pair<StringInput, Unit*> operator>>(StringInput si, Unit& m) {
		return pair<StringInput, Unit*> (si, &m);
	}
	friend StringInput operator>>(pair<StringInput, Unit*> si_m,
			const char* psD) {
		return si_m.first;
	}
};
...
private:
	X1 x1;
public:
	const X1& getX1() const {
		return x1;
	}
	size_t getHashKey() const {
		return x1.getHashKey();
	}
	MapKey1D() {
	}
	MapKey1D(const X1& a1) {
		x1 = a1;
	}
	bool operator==(const MapKey1D<X1>& h) const {
		return (x1 == h.x1);
	}
	bool operator<(const MapKey1D<X1>& h) const {
		return (x1 < h.x1);
	}
	friend ostream& operator<<(ostream& os, const MapKey1D<X1>& k) {
		return os << k.x1;
	}
	friend String& operator<<(String& str, const MapKey1D<X1>& k) {
		return str << k.x1;
	}
	friend IStream operator>>(pair<IStream, MapKey1D<X1>*> is_m,
			const char* psD) {
	...
	// OBSOLETE!
	friend pair<StringInput, MapKey1D<X1>*> operator>>(StringInput si,
			MapKey1D<X1>& m) {
		return pair<StringInput, MapKey1D<X1>*> (si, &m);
	}
	friend StringInput operator>>(pair<StringInput, MapKey1D<X1>*> si_m,
			const char* psD) {
...
public:
	...
	const X2& getX2() const {
		return x2;
	}
	size_t getHashKey() const {
		size_t k = rotLeft(x1.getHashKey(), 3);
		k ^= x2.getHashKey();
		return k;
	}
	MapKey2D() {
	}
	MapKey2D(const X1& a1, const X2& a2) {
		x1 = a1;
		x2 = a2;
	}
	...
	bool operator<(const MapKey2D<X1, X2>& h) const {
		return ((x1 < h.x1) || (x1 == h.x1 && x2 < h.x2));
	}
	friend ostream& operator<<(ostream& os, const MapKey2D<X1, X2>& k) {
		return os << k.x1 << " " << k.x2;
	}
	friend String& operator<<(String& str, const MapKey2D<X1, X2>& k) {
		return str << k.x1 << " " << k.x2;
	}
	friend IStream operator>>(pair<IStream, MapKey2D<X1, X2>*> is_m,
			const char* psD) {
		MapKey2D<X1, X2>& m = *is_m.second;
		IStream is2;
		IStream is = is_m.first >> m.x1 >> " ";
		while ((is2 = is >> " ") != IStream())
			is = is2;
		return is >> m.x2 >> psD;
	}

	// OBSOLETE!
	friend pair<StringInput, MapKey2D<X1, X2>*> operator>>(StringInput si,
			MapKey2D<X1, X2>& m) {
		return pair<StringInput, MapKey2D<X1, X2>*> (si, &m);
	}
	friend StringInput operator>>(pair<StringInput, MapKey2D<X1, X2>*> si_m,
			const char* psD) {
		MapKey2D<X1, X2>& m = *si_m.second;
		StringInput si2;
...
public:
	...
	const X3& getX3() const {
		return x3;
	}
	size_t getHashKey() const {
		size_t k = rotLeft(x1.getHashKey(), 3);
		k = rotLeft(k ^ x2.getHashKey(), 3);
		k ^= x3.getHashKey();
		return k;
	}
	MapKey3D() {
	}
	MapKey3D(const X1& a1, const X2& a2, const X3& a3) {
		x1 = a1;
		x2 = a2;
		x3 = a3;
	}
	bool operator==(const MapKey3D<X1, X2, X3>& h) const {
		return (x1 == h.x1 && x2 == h.x2 && x3 == h.x3);
	}
	bool operator<(const MapKey3D<X1, X2, X3>& h) const {
		return ((x1 < h.x1) || (x1 == h.x1 && x2 < h.x2) || (x1 == h.x1 && x2
				== h.x2 && x3 < h.x3));
	}
	friend ostream& operator<<(ostream& os, const MapKey3D<X1, X2, X3>& k) {
		return os << k.x1 << " " << k.x2 << " " << k.x3;
	}
	friend String& operator<<(String& str, const MapKey3D<X1, X2, X3>& k) {
		return str << k.x1 << " " << k.x2 << " " << k.x3;
	}
	friend IStream operator>>(pair<IStream, MapKey3D<X1, X2, X3>*> is_m,
			const char* psD) {
		MapKey3D<X1, X2, X3>& m = *is_m.second;
		IStream is2;
		IStream is = is_m.first >> m.x1 >> " ";
		while ((is2 = is >> " ") != IStream())
			is = is2;
	...
	// OBSOLETE!
	friend pair<StringInput, MapKey3D<X1, X2, X3>*> operator>>(StringInput si,
			MapKey3D<X1, X2, X3>& m) {
		return pair<StringInput, MapKey3D<X1, X2, X3>*> (si, &m);
	}
	friend StringInput operator>>(
			pair<StringInput, MapKey3D<X1, X2, X3>*> si_m, const char* psD) {
		MapKey3D<X1, X2, X3>& m = *si_m.second;
		StringInput si2;
		StringInput si = si_m.first >> m.x1 >> " ";
		while ((si2 = si >> " ") != NULL)
			si = si2;
...
public:
	...
	const X4& getX4() const {
		return x4;
	}
	size_t getHashKey() const {
		size_t k = rotLeft(x1.getHashKey(), 3);
		k = rotLeft(k ^ x2.getHashKey(), 3);
		k = rotLeft(k ^ x3.getHashKey(), 3);
		k ^= x4.getHashKey();
		return k;
	}
	MapKey4D() {
	}
	MapKey4D(const X1& a1, const X2& a2, const X3& a3, const X4& a4) {
		x1 = a1;
		x2 = a2;
		x3 = a3;
		x4 = a4;
	...
	bool operator==(const MapKey4D<X1, X2, X3, X4>& h) const {
		return (x1 == h.x1 && x2 == h.x2 && x3 == h.x3 && x4 == h.x4);
	}
	bool operator<(const MapKey4D<X1, X2, X3, X4>& h) const {
		return ((x1 < h.x1) || (x1 == h.x1 && x2 < h.x2) || (x1 == h.x1 && x2
				== h.x2 && x3 < h.x3) || (x1 == h.x1 && x2 == h.x2 && x3
				== h.x3 && x4 < h.x4));
	}
	friend ostream& operator<<(ostream& os, const MapKey4D<X1, X2, X3, X4>& k) {
		return os << k.x1 << " " << k.x2 << " " << k.x3 << " " << k.x4;
	}
	friend String& operator<<(String& str, const MapKey4D<X1, X2, X3, X4>& k) {
		return str << k.x1 << " " << k.x2 << " " << k.x3 << " " << k.x4;
	}
	friend IStream operator>>(pair<IStream, MapKey4D<X1, X2, X3, X4>*> is_m,
			const char* psD) {
		MapKey4D<X1, X2, X3, X4>& m = *is_m.second;
		IStream is2;
		IStream is = is_m.first >> m.x1 >> " ";
		while ((is2 = is >> " ") != IStream())
			is = is2;
	...
	// OBSOLETE!
	friend pair<StringInput, MapKey4D<X1, X2, X3, X4>*> operator>>(
			StringInput si, MapKey4D<X1, X2, X3, X4>& m) {
		return pair<StringInput, MapKey4D<X1, X2, X3, X4>*> (si, &m);
	}
	friend StringInput operator>>(
			pair<StringInput, MapKey4D<X1, X2, X3, X4>*> si_m, const char* psD) {
		MapKey4D<X1, X2, X3, X4>& m = *si_m.second;
		StringInput si2;
		StringInput si = si_m.first >> m.x1 >> " ";
		while ((si2 = si >> " ") != NULL)
			si = si2;
...
public:
	...
	size_t getHashKey() const {
		...
		k = rotLeft(k ^ x2.getHashKey(), 3);
		k = rotLeft(k ^ x3.getHashKey(), 3);
		k = rotLeft(k ^ x4.getHashKey(), 3);
		k ^= x5.getHashKey();
		return k;
	}
	MapKey5D() {
	}
	MapKey5D(const X1& a1, const X2& a2, const X3& a3, const X4& a4,
			const X5& a5) {
		x1 = a1;
		x2 = a2;
		x3 = a3;
	...
	bool operator==(const MapKey5D<X1, X2, X3, X4, X5>& h) const {
		return (x1 == h.x1 && x2 == h.x2 && x3 == h.x3 && x4 == h.x4 && x5
				== h.x5);
	}
	bool operator<(const MapKey5D<X1, X2, X3, X4, X5>& h) const {
		return ((x1 < h.x1) || (x1 == h.x1 && x2 < h.x2) || (x1 == h.x1 && x2
				== h.x2 && x3 < h.x3) || (x1 == h.x1 && x2 == h.x2 && x3
				== h.x3 && x4 < h.x4) || (x1 == h.x1 && x2 == h.x2 && x3
				== h.x3 && x4 == h.x4 && x5 < h.x5));
	}
	friend ostream& operator<<(ostream& os,
			const MapKey5D<X1, X2, X3, X4, X5>& k) {
		return os << k.x1 << " " << k.x2 << " " << k.x3 << " " << k.x4 << " "
				<< k.x5;
	}
	...
	friend IStream operator>>(
			pair<IStream, MapKey5D<X1, X2, X3, X4, X5>*> is_m, const char* psD) {
		MapKey5D<X1, X2, X3, X4, X5>& m = *is_m.second;
		IStream is2;
		IStream is = is_m.first >> m.x1 >> " ";
		while ((is2 = is >> " ") != IStream())
			is = is2;
	...
	// OBSOLETE!
	friend pair<StringInput, MapKey5D<X1, X2, X3, X4, X5>*> operator>>(
			StringInput si, MapKey5D<X1, X2, X3, X4, X5>& m) {
		return pair<StringInput, MapKey5D<X1, X2, X3, X4, X5>*> (si, &m);
	}
	friend StringInput operator>>(
			pair<StringInput, MapKey5D<X1, X2, X3, X4, X5>*> si_m,
			const char* psD) {
		MapKey5D<X1, X2, X3, X4, X5>& m = *si_m.second;
		StringInput si2;
		StringInput si = si_m.first >> m.x1 >> " ";
		while ((si2 = si >> " ") != NULL)
			si = si2;
...
public:
	...
	size_t getHashKey() const {
		...
		k = rotLeft(k ^ x3.getHashKey(), 3);
		k = rotLeft(k ^ x4.getHashKey(), 3);
		k = rotLeft(k ^ x5.getHashKey(), 3);
		k ^= x6.getHashKey();
		return k;
	}
	MapKey6D() {
	}
	MapKey6D(const X1& a1, const X2& a2, const X3& a3, const X4& a4,
			const X5& a5, const X6& a6) {
		x1 = a1;
		x2 = a2;
		x3 = a3;
		x4 = a4;
	...
	bool operator<(const MapKey6D<X1, X2, X3, X4, X5, X6>& h) const {
		return ((x1 < h.x1) || (x1 == h.x1 && x2 < h.x2) || (x1 == h.x1 && x2
				== h.x2 && x3 < h.x3) || (x1 == h.x1 && x2 == h.x2 && x3
				== h.x3 && x4 < h.x4) || (x1 == h.x1 && x2 == h.x2 && x3
				== h.x3 && x4 == h.x4 && x5 < h.x5) || (x1 == h.x1 && x2
				== h.x2 && x3 == h.x3 && x4 == h.x4 && x5 == h.x5 && x6 < h.x6));
	}
	friend ostream& operator<<(ostream& os,
			const MapKey6D<X1, X2, X3, X4, X5, X6>& k) {
		return os << k.x1 << " " << k.x2 << " " << k.x3 << " " << k.x4 << " "
				<< k.x5 << " " << k.x6;
	}
	friend String& operator<<(String& str,
	...
	friend IStream operator>>(
			pair<IStream, MapKey6D<X1, X2, X3, X4, X5, X6>*> is_m,
			const char* psD) {
		MapKey6D<X1, X2, X3, X4, X5, X6>& m = *is_m.second;
		IStream is2;
		IStream is = is_m.first >> m.x1 >> " ";
		while ((is2 = is >> " ") != IStream())
			is = is2;
	...
	// OBSOLETE!
	friend pair<StringInput, MapKey6D<X1, X2, X3, X4, X5, X6>*> operator>>(
			StringInput si, MapKey6D<X1, X2, X3, X4, X5, X6>& m) {
		return pair<StringInput, MapKey6D<X1, X2, X3, X4, X5, X6>*> (si, &m);
	}
	friend StringInput operator>>(
			pair<StringInput, MapKey6D<X1, X2, X3, X4, X5, X6>*> si_m,
			const char* psD) {
		MapKey6D<X1, X2, X3, X4, X5, X6>& m = *si_m.second;
		StringInput si2;
		StringInput si = si_m.first >> m.x1 >> " ";
		while ((si2 = si >> " ") != NULL)
			si = si2;
...
public:
	...
	SimpleMap(const SimpleMap& s) :
		OrigMap(s) {
	}
	// Specification methods...
	Y& set(const X& x) {
		return OrigMap::operator[](x);
	}
	// Extraction methods...
	const Y& get(const X& x) const {
		return (OrigMap::end() != OrigMap::find(x)) ? OrigMap::find(x)->second
				: yDummy;
	}
	bool contains(const X& x) const {
		return (OrigMap::end() != OrigMap::find(x));
	}
	friend ostream& operator<<(ostream& os, const SimpleMap<X, Y>& h) {
		for (typename SimpleMap<X, Y>::const_iterator it = h.begin(); it
				!= h.end(); it++)
			os << ((it == h.begin()) ? "" : ",") << it->first << ":"
					<< it->second;
		return os;
	}
};
...
public:
	...
	void normalize() {
		for (typename HKYP::iterator ik = HKYP::begin(); ik != HKYP::end(); ik++) {
			K k = ik->first;
			P p = P();
			for (typename distribution::iterator itd = ik->second.begin(); itd
					!= ik->second.end(); ++itd)
				p += itd->second;
			if (p != P())
				for (typename distribution::iterator itd = ik->second.begin(); itd
	...
	void dump(ostream& os, const char* psId) const {
		for (typename HKYP::const_iterator ik = HKYP::begin(); ik
				!= HKYP::end(); ik++) {
			K k = ik->first;
			const distribution& dist = ik->second;
			for (typename distribution::const_iterator itd = dist.begin(); itd
					!= dist.end(); ++itd) {
				const Y& y = itd->first;
				os << psId << " " << k << " : " << y << " = "
						<< getProb(y, k).toDouble() << "\n";
			}
		}
	}
	...
	/*   } */
	void clear() {
		MAP_CONTAINER<K, MAP_CONTAINER<Y, P> >::clear();
	}
	...
	friend IStream operator>>(
			pair<IStream, GenericCPTModel<Y, K, P, psLbl>*> is_m,
			const char* psD) {
		Y y;
		K k;
		IStream is, is1;
		GenericCPTModel<Y, K, P, psLbl>& m = *is_m.second;
		is = is_m.first;
		if (is == IStream())
			return is;
		is = is >> psLbl;
		while ((is1 = is >> " ") != IStream())
			is = is1;
		////cerr<<"reading k...\n";
		is = is >> k >> " ";
		////cerr<<"         ...k='"<<k<<"'\n";
	...
	// OBSOLETE!
	friend pair<StringInput, GenericCPTModel<Y, K, P, psLbl>*> operator>>(
			StringInput si, GenericCPTModel<Y, K, P, psLbl>& m) {
		return pair<StringInput, GenericCPTModel<Y, K, P, psLbl>*> (si, &m);
	}
	friend StringInput operator>>(
			pair<StringInput, GenericCPTModel<Y, K, P, psLbl>*> si_m,
			const char* psD) {
		Y y;
		K k;
		StringInput si, si2;
		GenericCPTModel<Y, K, P, psLbl>& m = *si_m.second;
		si = si_m.first;
		if (si == NULL)
			return si;
		while ((si2 = si >> " ") != NULL)
...
public:
	...
	bool contains(const Y& y) const {
		return GenericCPTModel<Y, MapKey1D<Unit> , P, psLbl>::contains(y,
				MapKey1D<Unit> (Unit()));
	}
	...
	P getProb(const Y& y) const {
		return GenericCPTModel<Y, MapKey1D<Unit> , P, psLbl>::getProb(y,
				MapKey1D<Unit> (Unit()));
	}
	const typename GenericCPTModel<Y, MapKey1D<Unit> , P, psLbl>::distribution& getDist() const {
		return GenericCPTModel<Y, MapKey1D<Unit> , P, psLbl>::get(
				MapKey1D<Unit> (Unit()));
	}
	P& setProb(const Y& y) {
		return GenericCPTModel<Y, MapKey1D<Unit> , P, psLbl>::setProb(y,
				MapKey1D<Unit> (Unit()));
	}
	bool readFields(Array<char*>& aps) {
		if (3 == aps.size()) {
			GenericCPTModel<Y, MapKey1D<Unit> , P, psLbl>::setProb(Y(aps[1]),
					MapKey1D<Unit> (Unit())) = atof(aps[2]);
			return true;
		}
		return false;
	}
	friend pair<IStream, ParentType*> operator>>(IStream is, ThisType& m) {
		return pair<IStream, ParentType*> (is, &m);
	}
};
...
public:
	...
	bool contains(const Y& y, const X1& x1) const {
		return GenericCPTModel<Y, MapKey1D<X1> , P, psLbl>::contains(y,
				MapKey1D<X1> (x1));
	}
	bool contains(const X1& x1) const {
		return GenericCPTModel<Y, MapKey1D<X1> , P, psLbl>::contains(
				MapKey1D<X1> (x1));
	}
	...
	P getProb(const Y& y, const X1& x1) const {
		return GenericCPTModel<Y, MapKey1D<X1> , P, psLbl>::getProb(y,
				MapKey1D<X1> (x1));
	}
	const typename GenericCPTModel<Y, MapKey1D<X1> , P, psLbl>::distribution& getDist(
			const X1& x1) const {
		return GenericCPTModel<Y, MapKey1D<X1> , P, psLbl>::get(
				MapKey1D<X1> (x1));
	}
	P& setProb(const Y& y, const X1& x1) {
		return GenericCPTModel<Y, MapKey1D<X1> , P, psLbl>::setProb(y,
				MapKey1D<X1> (x1));
	}
	bool readFields(Array<char*>& aps) {
		if (4 == aps.size()) {
			GenericCPTModel<Y, MapKey1D<X1> , P, psLbl>::setProb(Y(aps[2]),
					MapKey1D<X1> (aps[1])) = atof(aps[3]);
			return true;
		}
		return false;
	}
...
public:
	...
	bool contains(const Y& y, const X1& x1, const X2& x2) const {
		return GenericCPTModel<Y, MapKey2D<X1, X2> , P, psLbl>::contains(y,
				MapKey2D<X1, X2> (x1, x2));
	}
	bool contains(const X1& x1, const X2& x2) const {
		return GenericCPTModel<Y, MapKey2D<X1, X2> , P, psLbl>::contains(
				MapKey2D<X1, X2> (x1, x2));
	}
	...
	P getProb(const Y& y, const X1& x1, const X2& x2) const {
		return GenericCPTModel<Y, MapKey2D<X1, X2> , P, psLbl>::getProb(y,
				MapKey2D<X1, X2> (x1, x2));
	}
	const typename GenericCPTModel<Y, MapKey2D<X1, X2> , P, psLbl>::distribution& getDist(
			const X1& x1, const X2& x2) const {
		return GenericCPTModel<Y, MapKey2D<X1, X2> , P, psLbl>::get(
				MapKey2D<X1, X2> (x1, x2));
	}
	P& setProb(const Y& y, const X1& x1, const X2& x2) {
		return GenericCPTModel<Y, MapKey2D<X1, X2> , P, psLbl>::setProb(y,
				MapKey2D<X1, X2> (x1, x2));
	}
	bool readFields(Array<char*>& aps) {
		if (5 == aps.size()) {
			GenericCPTModel<Y, MapKey2D<X1, X2> , P, psLbl>::setProb(Y(aps[3]),
					MapKey2D<X1, X2> (aps[1], aps[2])) = atof(aps[4]);
			return true;
		}
		return false;
	}
...
////////////////////
template<class Y, class X1, class X2, class X3, class P, const char* psLbl =
		psNil>
...
public:
	...
	bool contains(const Y& y, const X1& x1, const X2& x2, const X3& x3) const {
		return GenericCPTModel<Y, MapKey3D<X1, X2, X3> , P, psLbl>::contains(y,
				MapKey3D<X1, X2, X3> (x1, x2, x3));
	}
	bool contains(const X1& x1, const X2& x2, const X3& x3) const {
		return GenericCPTModel<Y, MapKey3D<X1, X2, X3> , P, psLbl>::contains(
				MapKey3D<X1, X2, X3> (x1, x2, x3));
	}
	...
	P getProb(const Y& y, const X1& x1, const X2& x2, const X3& x3) const {
		return GenericCPTModel<Y, MapKey3D<X1, X2, X3> , P, psLbl>::getProb(y,
				MapKey3D<X1, X2, X3> (x1, x2, x3));
	}
	const typename GenericCPTModel<Y, MapKey3D<X1, X2, X3> , P, psLbl>::distribution& getDist(
			const X1& x1, const X2& x2, const X3& x3) const {
		return GenericCPTModel<Y, MapKey3D<X1, X2, X3> , P, psLbl>::get(
				MapKey3D<X1, X2, X3> (x1, x2, x3));
	}
	P& setProb(const Y& y, const X1& x1, const X2& x2, const X3& x3) {
		return GenericCPTModel<Y, MapKey3D<X1, X2, X3> , P, psLbl>::setProb(y,
				MapKey3D<X1, X2, X3> (x1, x2, x3));
	}
	bool readFields(Array<char*>& aps) {
		if (6 == aps.size()) {
			GenericCPTModel<Y, MapKey3D<X1, X2, X3> , P, psLbl>::setProb(
					Y(aps[4]), MapKey3D<X1, X2, X3> (aps[1], aps[2], aps[3]))
					= atof(aps[5]);
			return true;
		}
		return false;
...
public:
	...
	/*   } */
	bool contains(const Y& y, const X1& x1, const X2& x2, const X3& x3,
			const X4& x4) const {
		return GenericCPTModel<Y, MapKey4D<X1, X2, X3, X4> , P, psLbl>::contains(
				y, MapKey4D<X1, X2, X3, X4> (x1, x2, x3, x4));
	}
	bool contains(const X1& x1, const X2& x2, const X3& x3, const X4& x4) const {
		return GenericCPTModel<Y, MapKey4D<X1, X2, X3, X4> , P, psLbl>::contains(
				MapKey4D<X1, X2, X3, X4> (x1, x2, x3, x4));
	}
	...
	/*   } */
	P getProb(const Y& y, const X1& x1, const X2& x2, const X3& x3,
			const X4& x4) const {
		return GenericCPTModel<Y, MapKey4D<X1, X2, X3, X4> , P, psLbl>::getProb(
				y, MapKey4D<X1, X2, X3, X4> (x1, x2, x3, x4));
	}
	const typename GenericCPTModel<Y, MapKey4D<X1, X2, X3, X4> , P, psLbl>::distribution& getDist(
			const X1& x1, const X2& x2, const X3& x3, const X4& x4) const {
		return GenericCPTModel<Y, MapKey4D<X1, X2, X3, X4> , P, psLbl>::get(
				MapKey4D<X1, X2, X3, X4> (x1, x2, x3, x4));
	}
	P& setProb(const Y& y, const X1& x1, const X2& x2, const X3& x3,
			const X4& x4) {
		return GenericCPTModel<Y, MapKey4D<X1, X2, X3, X4> , P, psLbl>::setProb(
				y, MapKey4D<X1, X2, X3, X4> (x1, x2, x3, x4));
	}
	bool readFields(Array<char*>& aps) {
		if (7 == aps.size()) {
			GenericCPTModel<Y, MapKey4D<X1, X2, X3, X4> , P, psLbl>::setProb(
					Y(aps[5]),
					MapKey4D<X1, X2, X3, X4> (aps[1], aps[2], aps[3], aps[4]))
					= atof(aps[6]);
			return true;
		}
		return false;
...
template<class Y, class X1, class X2, class X3, class X4, class X5, class P,
		const char* psLbl = psNil>
class CPT6DModel: public GenericCPTModel<Y, MapKey5D<X1, X2, X3, X4, X5> , P,
		psLbl> {
	typedef GenericCPTModel<Y, MapKey5D<X1, X2, X3, X4, X5> , P, psLbl>
			ParentType;
...
public:
	...
	/*   } */
	bool contains(const Y& y, const X1& x1, const X2& x2, const X3& x3,
			const X4& x4, const X5& x5) const {
		return GenericCPTModel<Y, MapKey5D<X1, X2, X3, X4, X5> , P, psLbl>::contains(
				y, MapKey5D<X1, X2, X3, X4, X5> (x1, x2, x3, x4, x5));
	}
	bool contains(const X1& x1, const X2& x2, const X3& x3, const X4& x4,
			const X5& x5) const {
		return GenericCPTModel<Y, MapKey5D<X1, X2, X3, X4, X5> , P, psLbl>::contains(
				MapKey5D<X1, X2, X3, X4, X5> (x1, x2, x3, x4, x5));
	}
	...
	P getProb(const Y& y, const X1& x1, const X2& x2, const X3& x3,
			const X4& x4, const X5& x5) const {
		return GenericCPTModel<Y, MapKey5D<X1, X2, X3, X4, X5> , P, psLbl>::getProb(
				y, MapKey5D<X1, X2, X3, X4, X5> (x1, x2, x3, x4, x5));
	}
	const typename GenericCPTModel<Y, MapKey5D<X1, X2, X3, X4, X5> , P, psLbl>::distribution& getDist(
			const X1& x1, const X2& x2, const X3& x3, const X4& x4,
			const X5& x5) const {
		return GenericCPTModel<Y, MapKey5D<X1, X2, X3, X4, X5> , P, psLbl>::get(
				MapKey5D<X1, X2, X3, X4, X5> (x1, x2, x3, x4, x5));
	}
	P& setProb(const Y& y, const X1& x1, const X2& x2, const X3& x3,
			const X4& x4, const X5& x5) {
		return GenericCPTModel<Y, MapKey5D<X1, X2, X3, X4, X5> , P, psLbl>::setProb(
				y, MapKey5D<X1, X2, X3, X4, X5> (x1, x2, x3, x4, x5));
	}
	bool readFields(Array<char*>& aps) {
		if (8 == aps.size()) {
			GenericCPTModel<Y, MapKey5D<X1, X2, X3, X4, X5> , P, psLbl>::setProb(
					Y(aps[6]),
					MapKey5D<X1, X2, X3, X4, X5> (aps[1], aps[2], aps[3],
							aps[4], aps[5])) = atof(aps[7]);
			return true;
		}
		return false;
	}
...
template<class Y, class K, class P, const char* psLbl = psNil>
//template <class T> ... can be replaced by template <typename T> ...
//Dingcheng note: according to http://pages.cs.wisc.edu/~driscoll/typename.html This means exactly the same thing as the previous instance.
//The typename and class keywords can be used interchangeably to state that
...
public:
	...
	//Before a qualified dependent type, you need typename Dingcheng Notes.
	typedef typename Y::template ArrayIterator<P> IterVal;

	typename HKYP::const_iterator begin() const {
		return HKYP::begin();
	}
	typename HKYP::iterator begin() {
		return HKYP::begin();
	}
	typename Y::template ArrayIterator<P> begin(const K& k) const {
		typename Y::template ArrayIterator<P> iyp;
	...
	P setIterProb(typename Y::template ArrayIterator<P>& iyp, const K& k,int& vctr) const {
		...
		if (vctr < 0) {
		//	cout<<" what is iyp before HKYP::get(k): "<<iyp<<endl;
			iyp.first = HKYP::get(k);
			iyp.second = 0;
		//	cout<<" iyp: "<<iyp<<" k: "<<k<<" "<<iyp.first.getRef().get(0).second<<" vctr: "<<vctr<<endl;
			pr = (iyp.first.getRef().get(0).second != P()) ? P(1.0) : P();
			if (pr == P()) {
				cerr << "ERROR: Some condition has no value!  Key = '" << k
						<< "' value: '"<<iyp << "' vctr=" << vctr  << "\n";
			}
			//return P(1.0);
		}
		...
		// NOTE: falling off the end of a distribution is not a model error; only failing to start a new distribution!
			...
			if (iyp.second + 1 < iyp.first.getRef().getSize()) {
				iyp.second = iyp.second + 1;
	...
	friend IStream operator>>(
			pair<IStream, GenericHidVarCPTModel<Y, K, P, psLbl>*> is_m,
			const char* psD) {
		Y y;
		K k;
		IStream is, is1;
		GenericHidVarCPTModel<Y, K, P, psLbl>& m = *is_m.second;
		is = is_m.first;
		if (is == IStream())
			return is;
		is = is >> psLbl;
	...
	// OBSOLETE!
	friend pair<StringInput, GenericHidVarCPTModel<Y, K, P, psLbl>*> operator>>(
			StringInput si, GenericHidVarCPTModel<Y, K, P, psLbl>& m) {
		return pair<StringInput, GenericHidVarCPTModel<Y, K, P, psLbl>*> (si,
				&m);
	}
	friend StringInput operator>>(
			pair<StringInput, GenericHidVarCPTModel<Y, K, P, psLbl>*> si_m,
			const char* psD) {
		Y y;
		K k;
		StringInput si, si2;
		GenericHidVarCPTModel<Y, K, P, psLbl>& m = *si_m.second;
		si = si_m.first;
		if (si == NULL)
			return si;
		while ((si2 = si >> " ") != NULL)
...
////////////////////
template<class Y, class P, const char* psLbl = psNil>
class HidVarCPT1DModel: public GenericHidVarCPTModel<Y, MapKey1D<Unit> , P,
		psLbl> {
...
public:
	...
	HidVarCPT1DModel(const Y& y) {
		setProb(y) = P(1.0);
	}
	P setIterProb(typename Y::template ArrayIterator<P>& iyp, int& vctr) const {
		return GenericHidVarCPTModel<Y, MapKey1D<Unit> , P, psLbl>::setIterProb(
				iyp, MapKey1D<Unit> (), vctr);
	}
	P& setProb(const Y& y) {
		return GenericHidVarCPTModel<Y, MapKey1D<Unit> , P, psLbl>::setProb(y,
				MapKey1D<Unit> ());
	}
	P getProb(const typename Y::template ArrayIterator<P>& iyp) const {
		return GenericHidVarCPTModel<Y, MapKey1D<Unit> , P, psLbl>::getProb(
				iyp, MapKey1D<Unit> ());
	}
	const typename Y::template ArrayDistrib<P>& getDistrib() const {
		return GenericHidVarCPTModel<Y, MapKey1D<Unit> , P, psLbl>::getDistrib(
				MapKey1D<Unit> ());
	}
	bool contains() const {
		return GenericHidVarCPTModel<Y, MapKey1D<Unit> , P, psLbl>::contains(
				MapKey1D<Unit> ());
	}
	friend pair<IStream, ParentType*> operator>>(IStream is, ThisType& m) {
		return pair<IStream, ParentType*> (is, &m);
...
public:
	P setIterProb(typename Y::template ArrayIterator<P>& iyp, const X1& x1,
			int& vctr) const {
		return GenericHidVarCPTModel<Y, MapKey1D<X1> , P, psLbl>::setIterProb(
				iyp, MapKey1D<X1> (x1), vctr);
	}
	P& setProb(const Y& y, const X1& x1) {
		return GenericHidVarCPTModel<Y, MapKey1D<X1> , P, psLbl>::setProb(y,
				MapKey1D<X1> (x1));
	}
	typename Y::template ArrayDistrib<P>& setDistrib(const X1& x1) {
		return GenericHidVarCPTModel<Y, MapKey1D<X1> , P, psLbl>::setDistrib(
				MapKey1D<X1> (x1));
	}
	P getProb(const typename Y::template ArrayIterator<P>& iyp, const X1& x1) const {
		return GenericHidVarCPTModel<Y, MapKey1D<X1> , P, psLbl>::getProb(iyp,
				MapKey1D<X1> (x1));
	}
	const typename Y::template ArrayDistrib<P>& getDistrib(const X1& x1) const {
		return GenericHidVarCPTModel<Y, MapKey1D<X1> , P, psLbl>::getDistrib(
				MapKey1D<X1> (x1));
	}
	bool contains(const X1& x1) const {
		return GenericHidVarCPTModel<Y, MapKey1D<X1> , P, psLbl>::contains(
				MapKey1D<X1> (x1));
	}
	friend pair<IStream, ParentType*> operator>>(IStream is, ThisType& m) {
		return pair<IStream, ParentType*> (is, &m);
...
////////////////////
template<class Y, class X1, class X2, class P, const char* psLbl = psNil>
class HidVarCPT3DModel: public GenericHidVarCPTModel<Y, MapKey2D<X1, X2> , P,
		psLbl> {
	typedef GenericHidVarCPTModel<Y, MapKey2D<X1, X2> , P, psLbl> ParentType;
	typedef HidVarCPT3DModel<Y, X1, X2, P, psLbl> ThisType;
public:
	P setIterProb(typename Y::template ArrayIterator<P>& iyp, const X1& x1,
			const X2& x2, int& vctr) const {
		return GenericHidVarCPTModel<Y, MapKey2D<X1, X2> , P, psLbl>::setIterProb(
				iyp, MapKey2D<X1, X2> (x1, x2), vctr);
	}
	P& setProb(const Y& y, const X1& x1, const X2& x2) {
		return GenericHidVarCPTModel<Y, MapKey2D<X1, X2> , P, psLbl>::setProb(
				y, MapKey2D<X1, X2> (x1, x2));
	}
	typename Y::template ArrayDistrib<P>& setDistrib(const X1& x1, const X2& x2) {
		return GenericHidVarCPTModel<Y, MapKey2D<X1, X2> , P, psLbl>::setDistrib(
				MapKey2D<X1, X2> (x1, x2));
	}
	P getProb(const typename Y::template ArrayIterator<P>& iyp, const X1& x1,
			const X2& x2) const {
		return GenericHidVarCPTModel<Y, MapKey2D<X1, X2> , P, psLbl>::getProb(
				iyp, MapKey2D<X1, X2> (x1, x2));
	}
	const typename Y::template ArrayDistrib<P>& getDistrib(const X1& x1,
			const X2& x2) const {
		return GenericHidVarCPTModel<Y, MapKey2D<X1, X2> , P, psLbl>::getDistrib(
				MapKey2D<X1, X2> (x1, x2));
	}
	bool contains(const X1& x1, const X2& x2) const {
		return GenericHidVarCPTModel<Y, MapKey2D<X1, X2> , P, psLbl>::contains(
				MapKey2D<X1, X2> (x1, x2));
	}
	friend pair<IStream, ParentType*> operator>>(IStream is, ThisType& m) {
		return pair<IStream, ParentType*> (is, &m);
...
template<class Y, class X1, class X2, class X3, class P, const char* psLbl =
		psNil>
class HidVarCPT4DModel: public GenericHidVarCPTModel<Y, MapKey3D<X1, X2, X3> ,
		P, psLbl> {
	typedef GenericHidVarCPTModel<Y, MapKey3D<X1, X2, X3> , P, psLbl>
			ParentType;
	typedef HidVarCPT4DModel<Y, X1, X2, X3, P, psLbl> ThisType;
public:
	P setIterProb(typename Y::template ArrayIterator<P>& iyp, const X1& x1,
			const X2& x2, const X3& x3, int& vctr) const {
		return GenericHidVarCPTModel<Y, MapKey3D<X1, X2, X3> , P, psLbl>::setIterProb(
				iyp, MapKey3D<X1, X2, X3> (x1, x2, x3), vctr);
	}
	P& setProb(const Y& y, const X1& x1, const X2& x2, const X3& x3) {
		return GenericHidVarCPTModel<Y, MapKey3D<X1, X2, X3> , P, psLbl>::setProb(
				y, MapKey3D<X1, X2, X3> (x1, x2, x3));
	}
	typename Y::template ArrayDistrib<P>& setDistrib(const X1& x1,
			const X2& x2, const X3& x3) {
		return GenericHidVarCPTModel<Y, MapKey3D<X1, X2, X3> , P, psLbl>::setDistrib(
				MapKey3D<X1, X2, X3> (x1, x2, x3));
	}
	P getProb(const typename Y::template ArrayIterator<P>& iyp, const X1& x1,
			const X2& x2, const X3& x3) const {
		return GenericHidVarCPTModel<Y, MapKey3D<X1, X2, X3> , P, psLbl>::getProb(
				iyp, MapKey3D<X1, X2, X3> (x1, x2, x3));
	}
	const typename Y::template ArrayDistrib<P>& getDistrib(const X1& x1,
			const X2& x2, const X3& x3) const {
		return GenericHidVarCPTModel<Y, MapKey3D<X1, X2, X3> , P, psLbl>::getDistrib(
				MapKey3D<X1, X2, X3> (x1, x2, x3));
	}
	bool contains(const X1& x1, const X2& x2, const X3& x3) const {
		return GenericHidVarCPTModel<Y, MapKey3D<X1, X2, X3> , P, psLbl>::contains(
				MapKey3D<X1, X2, X3> (x1, x2, x3));
	}
	friend pair<IStream, ParentType*> operator>>(IStream is, ThisType& m) {
		return pair<IStream, ParentType*> (is, &m);
...
template<class Y, class X1, class X2, class X3, class X4, class P,
		const char* psLbl = psNil>
class HidVarCPT5DModel: public GenericHidVarCPTModel<Y,
		MapKey4D<X1, X2, X3, X4> , P, psLbl> {
	typedef GenericHidVarCPTModel<Y, MapKey4D<X1, X2, X3, X4> , P, psLbl>
			ParentType;
	typedef HidVarCPT5DModel<Y, X1, X2, X3, X4, P, psLbl> ThisType;
public:
	P setIterProb(typename Y::template ArrayIterator<P>& iyp, const X1& x1,
			const X2& x2, const X3& x3, const X4& x4, int& vctr) const {
		return GenericHidVarCPTModel<Y, MapKey4D<X1, X2, X3, X4> , P, psLbl>::setIterProb(
				iyp, MapKey4D<X1, X2, X3, X4> (x1, x2, x3, x4), vctr);
	}
	P& setProb(const Y& y, const X1& x1, const X2& x2, const X3& x3,
			const X4& x4) {
		return GenericHidVarCPTModel<Y, MapKey4D<X1, X2, X3, X4> , P, psLbl>::setProb(
				y, MapKey4D<X1, X2, X3, X4> (x1, x2, x3, x4));
	}
	typename Y::template ArrayDistrib<P>& setDistrib(const X1& x1,
			const X2& x2, const X3& x3, const X4& x4) {
		return GenericHidVarCPTModel<Y, MapKey4D<X1, X2, X3, X4> , P, psLbl>::setDistrib(
				MapKey4D<X1, X2, X3, X4> (x1, x2, x3, x4));
	}
	P getProb(const typename Y::template ArrayIterator<P>& iyp, const X1& x1,
			const X2& x2, const X3& x3, const X4& x4) const {
		return GenericHidVarCPTModel<Y, MapKey4D<X1, X2, X3, X4> , P, psLbl>::getProb(
				iyp, MapKey4D<X1, X2, X3, X4> (x1, x2, x3, x4));
	}
	const typename Y::template ArrayDistrib<P>& getDistrib(const X1& x1,
			const X2& x2, const X3& x3, const X4& x4) const {
		return GenericHidVarCPTModel<Y, MapKey4D<X1, X2, X3, X4> , P, psLbl>::getDistrib(
				MapKey4D<X1, X2, X3, X4> (x1, x2, x3, x4));
	}
	bool contains(const X1& x1, const X2& x2, const X3& x3, const X4& x4) const {
		return GenericHidVarCPTModel<Y, MapKey4D<X1, X2, X3, X4> , P, psLbl>::contains(
				MapKey4D<X1, X2, X3, X4> (x1, x2, x3, x4));
	}
	friend pair<IStream, ParentType*> operator>>(IStream is, ThisType& m) {
		return pair<IStream, ParentType*> (is, &m);
...
template<class Y, class X1, class X2, class X3, class X4, class X5, class P,
		const char* psLbl = psNil>
class HidVarCPT6DModel: public GenericHidVarCPTModel<Y, MapKey5D<X1, X2, X3,
		X4, X5> , P, psLbl> {
	typedef GenericHidVarCPTModel<Y, MapKey5D<X1, X2, X3, X4, X5> , P, psLbl>
			ParentType;
	typedef HidVarCPT6DModel<Y, X1, X2, X3, X4, X5, P, psLbl> ThisType;
public:
	P setIterProb(typename Y::template ArrayIterator<P>& iyp, const X1& x1,
			const X2& x2, const X3& x3, const X4& x4, const X5& x5, int& vctr) const {
		return GenericHidVarCPTModel<Y, MapKey5D<X1, X2, X3, X4, X5> , P, psLbl>::setIterProb(
				iyp, MapKey5D<X1, X2, X3, X4, X5> (x1, x2, x3, x4, x5), vctr);
	}
	P& setProb(const Y& y, const X1& x1, const X2& x2, const X3& x3,
			const X4& x4, const X5& x5) {
		return GenericHidVarCPTModel<Y, MapKey5D<X1, X2, X3, X4, X5> , P, psLbl>::setProb(
				y, MapKey5D<X1, X2, X3, X4, X5> (x1, x2, x3, x4, x5));
	}
	typename Y::template ArrayDistrib<P>& setDistrib(const X1& x1,
			const X2& x2, const X3& x3, const X4& x4, const X5& x5) {
		return GenericHidVarCPTModel<Y, MapKey5D<X1, X2, X3, X4, X5> , P, psLbl>::setDistrib(
				MapKey5D<X1, X2, X3, X4, X5> (x1, x2, x3, x4, x5));
	}
	P getProb(const typename Y::template ArrayIterator<P>& iyp, const X1& x1,
			const X2& x2, const X3& x3, const X4& x4, const X5& x5) const {
		return GenericHidVarCPTModel<Y, MapKey5D<X1, X2, X3, X4, X5> , P, psLbl>::getProb(
				iyp, MapKey5D<X1, X2, X3, X4, X5> (x1, x2, x3, x4, x5));
	}
	const typename Y::template ArrayDistrib<P>& getDistrib(const X1& x1,
			const X2& x2, const X3& x3, const X4& x4, const X5& x5) const {
		return GenericHidVarCPTModel<Y, MapKey5D<X1, X2, X3, X4, X5> , P, psLbl>::getDistrib(
				MapKey5D<X1, X2, X3, X4, X5> (x1, x2, x3, x4, x5));
	}
	bool contains(const X1& x1, const X2& x2, const X3& x3, const X4& x4,
			const X5& x5) const {
		return GenericHidVarCPTModel<Y, MapKey5D<X1, X2, X3, X4, X5> , P, psLbl>::contains(
				MapKey5D<X1, X2, X3, X4, X5> (x1, x2, x3, x4, x5));
	}
	friend pair<IStream, ParentType*> operator>>(IStream is, ThisType& m) {
		return pair<IStream, ParentType*> (is, &m);
...
template<class Y, class X1, class X2, class X3, class X4, class X5, class X6,
		class P, const char* psLbl = psNil>
class HidVarCPT7DModel: public GenericHidVarCPTModel<Y, MapKey6D<X1, X2, X3,
		X4, X5, X6> , P, psLbl> {
	typedef GenericHidVarCPTModel<Y, MapKey6D<X1, X2, X3, X4, X5, X6> , P,
			psLbl> ParentType;
	typedef HidVarCPT7DModel<Y, X1, X2, X3, X4, X5, X6, P, psLbl> ThisType;
public:
	P setIterProb(typename Y::template ArrayIterator<P>& iyp, const X1& x1,
			const X2& x2, const X3& x3, const X4& x4, const X5& x5,
			const X6& x6, int& vctr) const {
		return GenericHidVarCPTModel<Y, MapKey6D<X1, X2, X3, X4, X5, X6> , P,
				psLbl>::setIterProb(iyp,
				MapKey6D<X1, X2, X3, X4, X5, X6> (x1, x2, x3, x4, x5, x6), vctr);
	}
	P& setProb(const Y& y, const X1& x1, const X2& x2, const X3& x3,
			const X4& x4, const X5& x5, const X6& x6) {
		return GenericHidVarCPTModel<Y, MapKey6D<X1, X2, X3, X4, X5, X6> , P,
				psLbl>::setProb(y,
				MapKey6D<X1, X2, X3, X4, X5, X6> (x1, x2, x3, x4, x5, x6));
	}
	typename Y::template ArrayDistrib<P>& setDistrib(const X1& x1,
			const X2& x2, const X3& x3, const X4& x4, const X5& x5,
			const X6& x6) {
...
private:
	M1 m1;
public:
	const M1& getM1() const {
		return m1;
	}
	M1& setM1() {
		return m1;
	}
	void subsume(SingleFactoredModel<M1>& fm) {
		m1.subsume(fm.m1);
	}
	void clear() {
		m1.clear();
	...
	bool readFields(Array<char*>& aps) {
		return (m1.readFields(aps));
	}
	friend pair<StringInput, SingleFactoredModel<M1>*> operator>>(
			StringInput si, SingleFactoredModel<M1>& m) {
		return pair<StringInput, SingleFactoredModel<M1>*> (si, &m);
	}
	friend StringInput operator>>(
			pair<StringInput, SingleFactoredModel<M1>*> si_m, const char* psD) {
...
public:
	...
	void subsume(DoubleFactoredModel<M1, M2>& fm) {
		m1.subsume(fm.m1);
		m2.subsume(fm.m2);
	}
	void clear() {
	...
	bool readFields(Array<char*>& aps) {
		return (m1.readFields(aps) || m2.readFields(aps));
	}
	friend pair<StringInput, DoubleFactoredModel<M1, M2>*> operator>>(
			StringInput si, DoubleFactoredModel<M1, M2>& m) {
		return pair<StringInput, DoubleFactoredModel<M1, M2>*> (si, &m);
	}
	friend StringInput operator>>(
			pair<StringInput, DoubleFactoredModel<M1, M2>*> si_m,
			const char* psD) {
		StringInput si;
		return ((si = si_m.first >> si_m.second->m1 >> psD) != NULL || (si
				= si_m.first >> si_m.second->m2 >> psD) != NULL) ? si : NULL;
	}
};
...
public:
	...
	void subsume(TripleFactoredModel<M1, M2, M3>& fm) {
		m1.subsume(fm.m1);
		m2.subsume(fm.m2);
		m3.subsume(fm.m3);
	}
	...
	bool readFields(Array<char*>& aps) {
		return (m1.readFields(aps) || m2.readFields(aps) || m3.readFields(aps));
	}
	friend pair<StringInput, TripleFactoredModel<M1, M2, M3>*> operator>>(
			StringInput si, TripleFactoredModel<M1, M2, M3>& m) {
		return pair<StringInput, TripleFactoredModel<M1, M2, M3>*> (si, &m);
	}
	friend StringInput operator>>(
			pair<StringInput, TripleFactoredModel<M1, M2, M3>*> si_m,
			const char* psD) {
		StringInput si;
		return ((si = si_m.first >> si_m.second->m1 >> psD) != NULL || (si
				= si_m.first >> si_m.second->m2 >> psD) != NULL || (si
				= si_m.first >> si_m.second->m3 >> psD) != NULL) ? si : NULL;
	}
};
...
public:
	...
	void subsume(QuadrupleFactoredModel<M1, M2, M3, M4>& fm) {
		m1.subsume(fm.m1);
		m2.subsume(fm.m2);
		m3.subsume(fm.m3);
		m4.subsum(fm.m4);
	...
	friend pair<StringInput, QuadrupleFactoredModel<M1, M2, M3, M4>*> operator>>(
			StringInput si, QuadrupleFactoredModel<M1, M2, M3, M4>& m) {
		return pair<StringInput, QuadrupleFactoredModel<M1, M2, M3, M4>*> (si,
				&m);
	}
	friend StringInput operator>>(
			pair<StringInput, QuadrupleFactoredModel<M1, M2, M3, M4>*> si_m,
			const char* psD) {
		StringInput si;
		return ((si = si_m.first >> si_m.second->m1 >> psD) != NULL || (si
				= si_m.first >> si_m.second->m2 >> psD) != NULL || (si
				= si_m.first >> si_m.second->m3 >> psD) != NULL || (si
				= si_m.first >> si_m.second->m4 >> psD) != NULL) ? si : NULL;
	}
};

template<class M1, class M2, class M3, class M4, class M5>
class PentupleFactoredModel {
private:
	M1 m1;
	M2 m2;
	M3 m3;
...
public:
	...
	void subsume(PentupleFactoredModel<M1, M2, M3, M4, M5>& fm) {
		m1.subsume(fm.m1);
		m2.subsume(fm.m2);
		m3.subsume(fm.m3);
		m4.subsum(fm.m4);
	...
	bool readFields(Array<char*>& aps) {
		return (m1.readFields(aps) || m2.readFields(aps) || m3.readFields(aps)
				|| m4.readFields(aps) || m5.readFields(aps));
	}
	friend pair<StringInput, PentupleFactoredModel<M1, M2, M3, M4, M5>*> operator>>(
			StringInput si, PentupleFactoredModel<M1, M2, M3, M4, M5>& m) {
		return pair<StringInput, PentupleFactoredModel<M1, M2, M3, M4, M5>*> (
				si, &m);
	}
	friend StringInput operator>>(
			pair<StringInput, PentupleFactoredModel<M1, M2, M3, M4, M5>*> si_m,
			const char* psD) {
		StringInput si;
		return ((si = si_m.first >> si_m.second->m1 >> psD) != NULL || (si
				= si_m.first >> si_m.second->m2 >> psD) != NULL || (si
				= si_m.first >> si_m.second->m3 >> psD) != NULL || (si
				= si_m.first >> si_m.second->m4 >> psD) != NULL || (si
				= si_m.first >> si_m.second->m5 >> psD) != NULL) ? si : NULL;
	}
};
