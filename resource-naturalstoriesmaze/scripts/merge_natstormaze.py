...
argparser = argparse.ArgumentParser(description='Generates naturalstoriesmaze.evmeasures data table.')
argparser.add_argument('f1', type=str, nargs=1, help='Path to itemmeasures data table')
args, unknown = argparser.parse_known_args()

def main():
    data1 = pd.read_csv(sys.stdin,sep=' ',skipinitialspace=True)
    data2 = pd.read_csv(args.f1[0],sep=' ',skipinitialspace=True)

    no_dups = [c for c in data2.columns.values if c not in data1.columns.values] + ['item', 'zone']
    ...
    for s in data1['subject'].unique():
        ...
        merged = pd.merge(data1_s, data2, how='inner', on=['item', 'zone'])
        merged['subject'] = s
