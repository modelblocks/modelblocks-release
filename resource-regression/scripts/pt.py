...
if __name__ == '__main__':
    ...
    argparser.add_argument('-y', '--obs', nargs='+', help='Paths to true/observed response data (needed only if mode == "corr").')
    ...
    for i in range(len(paths)):
        for j in range(i+1, len(paths)):
            ...
            if is_nested:
                a_data = pd.read_csv(a, header=None)
                b_data = pd.read_csv(b, header=None)

                if args.mode == 'corr':
                    y = pd.read_csv(y[j], header=None)
                    y = zscore(y)
