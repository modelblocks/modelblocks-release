...
def _EvalModel(dataset):
  ...
  sum_den = 0.0
  perplexity = 0.0
  print("word totsurp jlstmunk")
  for i, (inputs, char_inputs, _, targets, weights, target_words) in enumerate(data_gen):
    ...
    if np.isnan(log_perp):
      ...
      # weights.mean() is necessary for "partially full" batches
      sum_num += log_perp * weights.mean()
      sum_den += weights.mean()
      surp = np.log2(np.exp(log_perp))
      lstmunk = 1 if dataset._vocab.id_to_word(targets[0,0]) == "<UNK>" else 0

      if target_words[0] != "</S>":
          print(target_words[0] + " " + str(surp) + " " + str(lstmunk))
      # sys.stderr.write('log_perp: %f, surp: %f, weights.mean(): %f.\n' %
      #                  (log_perp, surp, weights.mean()))
    if sum_den > 0:
      perplexity = np.exp(sum_num / sum_den)

    # TODO: think about how to do "memory-resetting"
    sys.stderr.write('EvalStep: %d | LogPerp: %f | MeanWeight: %f | AvgPerp: %f | Word: %s\n' %
                     (i, log_perp, weights.mean(), perplexity, dataset._vocab.id_to_word(targets[0,0])))
