...
class Vocabulary(object):
  ...
  def word_to_id(self, word):
    ...
    # hacked to accommodate tokenization scheme in Dundee (-LRB-, -RRB-, n't)
    elif word == "-LRB-":
      return self._word_to_id["("]
    elif word == "-RRB-":
      return self._word_to_id[")"]
    elif word == "n't":
      return self._word_to_id["not"]
    return self.unk
...
def get_batch(generator, batch_size, num_steps, max_word_length, pad=False):
  ...
  while True:
    ...
    for i in range(batch_size):
      ...
      while cur_pos < num_steps:
        if cur_stream[i] is None or len(cur_stream[i][0]) <= 1:
          try:
            cur_stream[i] = list(generator.__next__())
        ...
        target_words = cur_stream[i][3][1:how_many+1]
    ...
    if no_more_data and np.sum(weights) == 0:
      ...
      break
    yield inputs, char_inputs, global_word_ids, targets, weights, target_words
...
class LM1BDataset(object):
  ...
  def _load_shard(self, shard_name):
    ...
    chars_ids = [self.vocab.encode_chars(sentence) for sentence in sentences if sentence.strip() != "!ARTICLE"]
    ids = [self.vocab.encode(sentence) for sentence in sentences if sentence.strip() != "!ARTICLE"]
    words = [['<S>'] + sentence.split() + ['</S>'] for sentence in sentences if sentence.strip() != "!ARTICLE"]
    ...
    return zip(ids, chars_ids, global_word_ids, words)
