...
def train(use_dev, dev_decpars_file, use_gpu, syn_size, sem_size, hidden_dim, num_epochs, batch_size, learning_rate, l2_reg):
    ...
    while True:
        ...
        eprint("batchsize: {}".format(batch_size))
        eprint("len(depth): {}".format(len(depth)))
        eprint("ttcorrect: {}".format(total_train_correct))
        eprint("Epoch {:04d} | AvgTrainLoss {:.4f} | TrainAcc {:.4f} | DevLoss {:.4f} | DevAcc {:.4f}".
               format(epoch, total_train_loss/((len(depth)//batch_size)+1), 100*(total_train_correct/len(depth)), total_dev_loss, dev_acc))
