import sys, re, os
from collections import Counter
import torch
import torch.nn as nn
import torch.nn.functional as F
import torch.optim as optim
import numpy as np
import argparse


def eprint(*args, **kwargs):
    print(*args, file=sys.stderr, **kwargs)


def prepare_data(decpars_file, use_dev):
    with open(decpars_file, "r") as f:
        data = f.readlines()
        data = [line.strip() for line in data]

    depth, catBase, hvBase, hvFiller, fDecs, hvBFirst, hvFFirst = ([] for i in range(7))

    for line in data:
        d, cb, hvb, hvf, fd = line.split(" ")
        depth.append(int(d))
        catBase.append(cb)
        hvBase.append(hvb)
        hvFiller.append(hvf)
        fDecs.append(fd)
    eprint("linesplit complete")

    for kvec in hvBase:
        match = re.findall(r"^\[(.*?)\]", kvec)
        hvBFirst.append([float(i) for i in match[0].split(",")])
    eprint("hvBase ready")

    for kvec in hvFiller:
        match = re.findall(r"^\[(.*?)\]", kvec)
        hvFFirst.append([float(i) for i in match[0].split(",")])
    eprint("hvFiller ready")

    cat_to_ix = {cat: i for i, cat in enumerate(sorted(set(catBase)))}
    fdecs_to_ix = {fdecs: i for i, fdecs in enumerate(sorted(set(fDecs)))}

    if use_dev >= 0:
        cat_to_ix["UNK"] = len(cat_to_ix)
        fdecs_to_ix["UNK"] = len(fdecs_to_ix)

    cat_b_ix, fdecs_ix = ([] for i in range(2))

    for cat in catBase:
        cat_b_ix.append(cat_to_ix[cat])

    for fdecs in fDecs:
        fdecs_ix.append(fdecs_to_ix[fdecs])

    ix_to_cat = {k: v for v, k in cat_to_ix.items()}
    ix_to_fdecs = {k: v for v, k in fdecs_to_ix.items()}

    eprint("number of output F categories: " + str(len(ix_to_fdecs)))

    return depth, cat_b_ix, hvBFirst, hvFFirst, ix_to_cat, fdecs_ix, ix_to_fdecs


def prepare_data_dev(dev_decpars_file, ix_to_cat, ix_to_fdecs):
    with open(dev_decpars_file, "r") as f:
        data = f.readlines()
        data = [line.strip() for line in data]

    depth, catBase, hvBase, hvFiller, fDecs, hvBFirst, hvFFirst = ([] for i in range(7))

    for line in data:
        d, cb, hvb, hvf, fd = line.split(" ")
        depth.append(int(d))
        catBase.append(cb)
        hvBase.append(hvb)
        hvFiller.append(hvf)
        fDecs.append(fd)

    for kvec in hvBase:
        match = re.findall(r"^\[(.*?)\]", kvec)
        hvBFirst.append([float(i) for i in match[0].split(",")])

    for kvec in hvFiller:
        match = re.findall(r"^\[(.*?)\]", kvec)
        hvFFirst.append([float(i) for i in match[0].split(",")])

    cat_to_ix = {k: v for v, k in ix_to_cat.items()}
    fdecs_to_ix = {k: v for v, k in ix_to_fdecs.items()}
    cat_b_ix, fdecs_ix = ([] for i in range(2))

    for cat in catBase:
        cat_b_ix.append(cat_to_ix.get(cat, cat_to_ix["UNK"]))

    for fdecs in fDecs:
        fdecs_ix.append(fdecs_to_ix.get(fdecs, fdecs_to_ix["UNK"]))

    return depth, cat_b_ix, hvBFirst, hvFFirst, fdecs_ix


# train models
class FModel(nn.Module):
    def __init__(self, cat_vocab_size, syn_size, sem_size, hidden_dim, output_dim):
        super(FModel, self).__init__()
        self.cat_embeds = nn.Embedding(cat_vocab_size, syn_size)
        self.hidden_dim = hidden_dim
        self.output_dim = output_dim
        self.fc1 = nn.Linear(7+syn_size+2*sem_size, self.hidden_dim, bias=False)
        self.relu = F.relu
        self.fc2 = nn.Linear(self.hidden_dim, self.output_dim, bias=False)

    def forward(self, d_onehot, cat_b_ix, hvb, hvf, use_gpu):
        cat_b_embed = self.cat_embeds(cat_b_ix)
        if use_gpu >= 0:
            cat_b_embed = cat_b_embed.to("cuda")
        x = torch.cat((cat_b_embed, hvb, hvf, d_onehot), 1)
        x = self.fc1(x)
        x = self.relu(x)
        x = self.fc2(x)
        return F.log_softmax(x, dim=1)


def train(decpars_file, use_dev, dev_decpars_file, use_gpu, syn_size, sem_size, epochs, batch_size, learning_rate):
    depth, cat_b_ix, hvb, hvf, ix_to_cat, fdecs_ix, ix_to_fdecs = prepare_data(decpars_file, use_dev)
    depth = F.one_hot(torch.LongTensor(depth), 7).float()
    cat_b_ix = torch.LongTensor(cat_b_ix)
    hvb = torch.FloatTensor(hvb)
    hvf = torch.FloatTensor(hvf)
    target = torch.LongTensor(fdecs_ix)
    model = FModel(len(ix_to_cat), syn_size, sem_size, 7+syn_size+2*sem_size, len(ix_to_fdecs))

    if use_gpu >= 0:
        depth = depth.to("cuda")
        cat_b_ix = cat_b_ix.to("cuda")
        hvb = hvb.to("cuda")
        hvf = hvf.to("cuda")
        target = target.to("cuda")
        model = model.cuda()

    if use_dev >= 0:
        dev_depth, dev_cat_b_ix, dev_hvb, dev_hvf, dev_fdecs_ix = prepare_data_dev(dev_decpars_file, ix_to_cat, ix_to_fdecs)
        dev_depth = F.one_hot(torch.LongTensor(dev_depth), 7).float()
        dev_cat_b_ix = torch.LongTensor(dev_cat_b_ix)
        dev_hvb = torch.FloatTensor(dev_hvb)
        dev_hvf = torch.FloatTensor(dev_hvf)
        dev_target = torch.LongTensor(dev_fdecs_ix)

        if use_gpu >= 0:
            dev_depth = dev_depth.to("cuda")
            dev_cat_b_ix = dev_cat_b_ix.to("cuda")
            dev_hvb = dev_hvb.to("cuda")
            dev_hvf = dev_hvf.to("cuda")
            dev_target = dev_target.to("cuda")

    optimizer = optim.Adam(model.parameters(), lr=learning_rate)
    criterion = nn.NLLLoss()

    for epoch in range(epochs):
        permutation = torch.randperm(len(depth))
        trng_loss = 0
        cum_dev_loss = 0

        for i in range(0, len(depth), batch_size):
            indices = permutation[i:i+batch_size]
            batch_d, batch_c, batch_hvb, batch_hvf, batch_target = depth[indices], cat_b_ix[indices], hvb[indices], hvf[indices], target[indices]
            output = model(batch_d, batch_c, batch_hvb, batch_hvf, use_gpu)
            loss = criterion(output, batch_target)
            trng_loss = loss.item()
            #eprint('Epoch [%d/%d], Batch [%d/%d], Loss: %.4f' % (epoch+1, epochs, i//batch_size+1, len(depth)//batch_size+1, loss.item()))
            loss.backward()
            optimizer.step()
            optimizer.zero_grad()
            #print("foo")

        if use_dev >= 0:
            with torch.no_grad():
                dev_pred = model(dev_depth, dev_cat_b_ix, dev_hvb, dev_hvf, use_gpu)
                dev_loss = criterion(dev_pred, dev_target)
                cum_dev_loss += dev_loss.item()

        eprint("Epoch:", epoch+1, "Training loss:", trng_loss, "Dev loss:", cum_dev_loss)

    return model, ix_to_cat, ix_to_fdecs


def main(args):
    model, ix_to_cat, ix_to_fdecs = train(args.decpars_file, args.use_dev, args.dev_decpars_file, args.use_gpu, args.syn_size, args.sem_size, args.epochs, args.batch_size, args.learning_rate)

    if args.use_gpu >= 0:
        cat_embeds = list(model.parameters())[0].data.cpu().numpy()
        first_weights = list(model.parameters())[1].data.cpu().numpy()
        second_weights = list(model.parameters())[2].data.cpu().numpy()
    else:
        cat_embeds = list(model.parameters())[0].data.numpy()
        first_weights = list(model.parameters())[1].data.numpy()
        second_weights = list(model.parameters())[2].data.numpy()

    eprint(first_weights.shape, second_weights.shape)
    print("F F " + ",".join(map(str, first_weights.flatten().tolist())))
    print("F S " + ",".join(map(str, second_weights.flatten('F').tolist())))
    for key in ix_to_cat:
        print("C " + str(ix_to_cat[key]) + " [" + ",".join(map(str, cat_embeds[key])) + "]")
    for key in ix_to_fdecs:
        print("f " + str(key) + " " + str(ix_to_fdecs[key]))


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='F model trainer')
    parser.add_argument("-train", "--decpars-file", type=str, required=True, help="training data")
    parser.add_argument("--use-dev", type=int, default=-1, help="use validation")
    parser.add_argument("-dev", "--dev-decpars-file", type=str, help="validation data")
    parser.add_argument("--use-gpu", type=int, default=1, help="use gpu")
    parser.add_argument("--syn-size", type=int, default=10, help="syntax size")
    parser.add_argument("--sem-size", type=int, default=20, help="semantics size")
    parser.add_argument("--epochs", type=int, default=100, help="number of epochs")
    parser.add_argument("--batch-size", type=int, default=256, help="mini-batch size")
    parser.add_argument("--learning-rate", type=float, default=0.001,
                        help="learning rate")
    args = parser.parse_args()
    eprint(args)
    main(args)
