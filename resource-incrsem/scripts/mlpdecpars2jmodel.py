...
def prepare_data():
    ...
    cata_to_ix = {cat: i for i, cat in enumerate(sorted(set(catAncstr)))}
    catl_to_ix = {cat: i for i, cat in enumerate(sorted(set(catLchild)))}
    ...
    hvecl_to_ix = {hvec: i for i, hvec in enumerate(sorted(set(flat_hvL)))}

    # cat_a_ix = [cat_to_ix[cat] for cat in catAncstr]
    # cat_l_ix = [cat_to_ix[cat] for cat in catLchild]
    cat_a_ix = [cata_to_ix[cat] for cat in catAncstr]
    cat_l_ix = [catl_to_ix[cat] for cat in catLchild]
    ...
    for i, sublist in enumerate(hvLFirst):
        ...
        for hvec in sublist:
            if hvec == "Top":
                ...
                hvl_col.append(hvecl_to_ix[hvec])
    ...
    hvl_mat = csr_matrix((np.ones(len(hvl_row), dtype=np.int32), (hvl_row, hvl_col)),
                         shape=(len(hvLFirst), len(hvecl_to_ix)))
...
def prepare_data_dev(dev_decpars_file, cata_to_ix, catl_to_ix, jdecs_to_ix, hveca_to_ix, hvecf_to_ix, hvecl_to_ix):
    ...
    for i, sublist in enumerate(hvLFirst):
        ...
        for hvec in sublist:
            if hvec == "Top":
                top_count += 1
            # elif hvec == "" or hvec == "Bot" or hvec not in hvec_to_ix:
            elif hvec == "" or hvec == "Bot" or hvec not in hvecl_to_ix:
                continue
            else:
                hvl_row.append(i)
                # hvl_col.append(hvec_to_ix[hvec])
...
class JModel(nn.Module):
    ...
    def __init__(self, cata_vocab_size, catl_vocab_size, hveca_vocab_size, hvecf_vocab_size, hvecl_vocab_size, syn_size, sem_size, hidden_dim, output_dim, dropout_prob):
        ...
        self.cata_embeds = nn.Embedding(cata_vocab_size, syn_size)
        self.catl_embeds = nn.Embedding(catl_vocab_size, syn_size)
        ...
        self.hvecl_embeds = nn.Embedding(hvecl_vocab_size, sem_size)
    ...
    def forward(self, d_onehot, cat_a_ix, hva_mat, hvf_mat, cat_l_ix, hvl_mat, hva_top, hvf_top, hvl_top, use_gpu,
        ...
        if ablate_sem:
            if use_gpu >= 0:
                hva_embed = torch.zeros([hva_top.shape[0], self.sem_size], dtype=torch.float, device=torch.device("cuda")) + hva_top
                hvf_embed = torch.zeros([hvf_top.shape[0], self.sem_size], dtype=torch.float, device=torch.device("cuda")) + hvf_top
                hvl_embed = torch.zeros([hvl_top.shape[0], self.sem_size], dtype=torch.float, device=torch.device("cuda")) + hvl_top
            else:
...
def train(use_dev, dev_decpars_file, use_gpu, syn_size, sem_size, hidden_dim, dropout_prob, num_epochs, batch_size,
    ...
    if use_dev >= 0:
        ...
        dev_depth, dev_cat_a_ix, dev_hva_mat, dev_hvf_mat, dev_cat_l_ix, dev_hvl_mat, dev_jdecs_ix, dev_hva_top, dev_hvf_top, dev_hvl_top = prepare_data_dev(
            dev_decpars_file, cata_to_ix, catl_to_ix, jdecs_to_ix, hveca_to_ix, hvecf_to_ix, hvecl_to_ix)
