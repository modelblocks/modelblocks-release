...
//  }
  ...
  // Calc P,F...
  map<Delimited<B>,map<P,Double>> mP;
  map<DelimitedTrip<psX,D,psSpace,B,psSpace,P,psX>,map<F,Double>> mF;
    ...
    for( const auto& pd : dbmpd.second ) {
      ...
      for( const auto& wxd : gamma[DSC(d+1,'L',p)] )
        if( wxd.first.second==C("-") ) {
          mP[Delimited<B>(b)][p]                                     += plus[dbmpd.first][p] * wxd.second;
          mF[DelimitedTrip<psX,D,psSpace,B,psSpace,P,psX>(d,b,p)][1] += plus[dbmpd.first][p] * wxd.second;
        }
    }
  }
  for( const auto& dbmpd : zero ) {
    ...
    for( const auto& pd : dbmpd.second ) {
      ...
      for( const auto& wxd : gamma[DSC(d,'R',p)] )
        if( wxd.first.second==C("-") ) {
          mP[Delimited<B>(b)][p]                                     += zero[dbmpd.first][p] * wxd.second;
          mF[DelimitedTrip<psX,D,psSpace,B,psSpace,P,psX>(d,b,p)][0] += zero[dbmpd.first][p] * wxd.second;
        }
    }
  }
