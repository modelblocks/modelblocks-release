...
String getCorefId ( const Tree<LVU>& tr ) { //ej coref change
  ...
  if (string::npos != l.find("-n")) {
          std::cmatch cm;
          std::regex e (".*-n([0-9]+).*")); //get consecutive numbers after a "-n"
          std::regex_match (l, cm, e); 
          return cm[1];
  }
  else return "";
}
...
void calcContext ( const Tree<LVU>& tr, const arma::mat& D, const arma::mat& U, int s=1, int d=0, E e='N', L l=L(), map<string,KSet> annot2kset ) { //ej change coref TODO change all calls to include annot2kset arg
  static F          f;
  static E          eF;
  static Sign       aPretrm;
  static StoreState q;
  ...
  if ( tr.size()==1 && tr.front().size()==0 ) {
    ...
    String annot = getCorefId(tr); //ej coref change 
    if annot != "" { 
            KSet ksAnt = annot2kset[annot]; 
            annot2kset[currentloc] = ksAnt + KSet(k)//update annot2kset map with current k. TODO how to get current sentid, wordid?
    }
    else ksAnt = KSet (); //empty kset if no annotation

    // Print preterminal / fork-phase predictors...
    DelimitedList<psX,FPredictor,psComma,psX> lfp;  q.calcForkPredictors(lfp, ksAnt); //add additional kset argument, should be set of all antecedents in referent cluster. requires global map from annotation to ksets, as in {"0204":Kset ["Lord_"], "0207": KSet ["Lord_", "he_"]}, etc.
    cout<<"----"<<q<<endl;
...
//cout<<( mtttmG[ trip<T,T,T>( "A-aN", "R-aN", "A-aN-PRTRM" ) ] * arma::kron( mIdent, vOnes ) )<<endl;
  ...
  while ( cin && EOF!=cin.peek() ) {
    ...
    if( t.front().size() > 0 ) calcContext( t, arma::diagmat(vFirstHot), mIdent, annot2kset ); //ej coref change. TODO calcContext needs discourseline?
  }
