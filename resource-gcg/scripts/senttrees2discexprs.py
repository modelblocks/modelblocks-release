...
#   elif '-lV' in t.ch[0].c:  output = [ 'Pasv', 'x'+ t.sVar, translate( t.ch[0], Scopes, Anaphs, [ ['Trace','x'+t.sVar] ] + lsNolo ) ]
    ## Z4. Zero-head rule with quantificational adjunct...
    elif re.search( '^K\w*-lZ \w+-[ab]\w+-[ab]\{\w+-[ab]\w+\}$', form ):  output = [ '\\f', '\\q', '\\r', '\\s', translate( t.ch[0], Scopes, Anaphs, lsNolo ), ['\\n', 'Ratio=n', ['\\x', 'q', ['\\z', 'Equal', 'x', 'z'], Univ ], ['\\x', 'f', ['\\t', '\\u', 'Some', ['\\z', '^', [ 'Equal', 'x', 'z' ], [ 't', 'z' ] ], 'u' ], 'r', 's'] ], Univ ]
...
#  elif expr == 'SelfStore':  return( [ '\\v', '\\q', '\\r', '\\s', 'q', Univ, [ '\\x'+?????
  ...
  elif re.search( '@K\w*-[ab]\w+:', expr ):  return( [ '\\q', '\\r', '\\s', 'q', [ '\\n'+sVar, 'True' ], [ '\\n'+sVar, 'All', [ '\\m'+sVar, '^', [ expr[1:], 'n'+sVar, 'm'+sVar ], [ 'r', 'm'+sVar ] ], 's' ] ] )
  elif re.search( '@K\w*-[ab]\w+-[ab]\w+:', expr ):  return( [ '\\p', '\\q', '\\r', '\\s', 'q', [ '\\n'+sVar, 'True' ], [ '\\n'+sVar, 'p', [ '\\o'+sVar, 'True' ], [ '\\o'+sVar, 'All', [ '\\m'+sVar, '^', [ expr[1:], 'o'+sVar, 'n'+sVar, 'm'+sVar ], [ 'r', 'm'+sVar ] ], 's' ] ] ] )
  ## Possessive marker...
