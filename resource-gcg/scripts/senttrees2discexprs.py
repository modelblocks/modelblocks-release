...
#   elif '-lV' in t.ch[0].c:  output = [ 'Pasv', 'x'+ t.sVar, translate( t.ch[0], Scopes, Anaphs, WeakAccs, MaxProjs, MaxClauses, tSubst, sSubst, [ ['Trace','x'+t.sVar] ] + lsNolo ) ]
    ## Z16. Zero-head rule with proportion as quantificational adjunct: e.g. B-aN-a{B-aN} -> K-lZ 'twice'...
    elif re.search( '^K\w*((?:-[ghirv][^ ]*)?)-lZ [VIBLG]\w*-[ab]\w+-[ab]\{[VIBLG]\w*-[ab]\w+\}\\1$', form ):  output = [ '\\f', '\\q', '\\r', '\\s', translate( t.ch[0], Scopes, Anaphs, WeakAccs, MaxProjs, MaxClauses, tSubst, sSubst, lsNolo ), ['\\n', 'Count=', 'n', ['\\x', 'q', ['\\z', 'Equal', 'x', 'z'], Univ ], ['\\x', 'f', ['\\t', '\\u', 'Some', ['\\z', '^', [ 'Equal', 'x', 'z' ], [ 't', 'z' ] ], 'u' ], 'r', 's'] ], Univ ]
    ...
    elif re.search( '^[AR]c\w*-[ab]\w+((?:-[ghirv][^ ]*)?) [AR]\w*-[ab]\w+-[ab]\w+\\1$', form ):
...
def unpack( expr ):
  ...
  elif expr == 'Mod3':  return( [ '\\f', '\\g', '\\o', '\\p', '\\q', '\\r', '\\s', 'f', 'o', 'p', 'q', [ '\\x', '^', ['r', 'x' ], [ 'g', [ '\\t', '\\u', '^', ['t','x'], ['u','x'] ], Univ, Univ ] ], 's' ] )
...
#  elif expr == 'SelfStore':  return( [ '\\v', '\\q', '\\r', '\\s', 'q', Univ, [ '\\x'+?????
  ...
  ## Unpack terminals...
  ## Cardinal...
  elif expr.split(':')[0] == '@K':  return( [ '\\r', '\\s', 'All', [ '\\n'+sVar, '^', [ expr[1:], 'n'+sVar ], [ 'r', 'n'+sVar ] ], 's' ] )
  elif re.search( '@K\w*-[ab]\w+:', expr ):  return( [ '\\q', '\\r', '\\s', 'Some', [ '\\m'+sVar, '^', [ 'r', 'm'+sVar ], [ 'q', [ '\\n'+sVar, 'True' ], [ '\\n'+sVar, expr[1:], 'n'+sVar, 'm'+sVar ] ] ], 's' ] )
#  elif re.search( '@K\w*-[ab]\w+:', expr ):  return( [ '\\q', '\\r', '\\s', 'All', 'r', [ '\\m'+sVar, '^', [ 'q', [ '\\n'+sVar, 'True' ], [ '\\n'+sVar, expr[1:], 'n'+sVar, 'm'+sVar ] ], [ 's', 'm'+sVar ] ] ] )
  elif re.search( '@K\w*-[ab]\w+-[ab]\w+:', expr ):  return( [ '\\p', '\\q', '\\r', '\\s', 'Some', [ '\\m'+sVar, '^', [ 'r', 'm'+sVar ], [ 'q', [ '\\n'+sVar, 'True' ], [ '\\n'+sVar, 'p', [ '\\o'+sVar, 'True' ], [ '\\o'+sVar, expr[1:], 'o'+sVar, 'n'+sVar, 'm'+sVar ] ] ] ], 's' ] )
#  elif re.search( '@K\w*-[ab]\w+-[ab]\w+:', expr ):  return( [ '\\p', '\\q', '\\r', '\\s', 'All', 'r', [ '\\m'+sVar, '^', [ 'q', [ '\\n'+sVar, 'True' ], [ '\\n'+sVar, 'p', [ '\\o'+sVar, 'True' ], [ '\\o'+sVar, expr[1:], 'o'+sVar, 'n'+sVar, 'm'+sVar ] ] ], [ 's', 'm'+sVar ] ] ] )
  ## Possessive marker...
...
#  elif expr.split(':')[0] == '@I-aN-b{B-aN}':  return( [ '\\f', '\\q', '\\r', '\\s', 'f', 'q', [ '\\e', '^', [expr[1:],'e'], ['r','e'] ], 's' ] )
  ...
  elif re.search( '^@[DN]NGEN-[ri]N:', expr ):  return( [ '\\q', '\\r', '\\s', 'q', Univ, [ '\\x'+sVar, 'Gen', [ '\\e'+sVar, '^', [expr[1:],'e'+sVar,'x'+sVar], ['r','e'+sVar] ], 's' ] ] )
...
def isWellFormedProp( expr ):
  ...
  if isinstance( expr, list ) and len(expr) > 3 and isinstance( expr[0], str ) and expr[0][0] != '\\' and 'Ratio' in expr[0] and isinstance( expr[2], list ) and len(expr[2]) > 0 and expr[2][0][0]=='\\' and isinstance( expr[3], list ) and len(expr[3]) > 0 and expr[3][0][0]=='\\' and isWellFormedSet( expr[2] ) and isWellFormedSet( expr[3] ):  return True
  if isinstance( expr, list ) and len(expr) > 3 and isinstance( expr[0], str ) and expr[0][0] != '\\' and 'Count' in expr[0] and isinstance( expr[2], list ) and len(expr[2]) > 0 and expr[2][0][0]=='\\' and isinstance( expr[3], list ) and len(expr[3]) > 0 and expr[3][0][0]=='\\' and isWellFormedSet( expr[2] ) and isWellFormedSet( expr[3] ):  return True
  ## Predicate...
