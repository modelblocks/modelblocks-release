...
$NOTPSG = 0;
if ($opts{n}) {
  print STDERR "Running non-gcg mode\n";
  $NOTPSG = 1;
} else {
  print STDERR "Running gcg mode\n";
}
...
while ( <> ) {
  ...
  if (!$opts{c}) {
    ...
    s/\(WHNP \(WDT which\) \) \(S  \(NP-SBJ-1 \(DT the\)/\(WHNP-1 \(WDT which\) \) \(S  \(NP-SBJ \(DT the\)/;
    # line 3670
    s/\(VBN entwined\)  \(PP-CLR \(-NONE- \*-2\) \)\)/\(VBN entwined\)  \(PP-CLR \(-NONE- \*T\*-2\) \)\)/;
    # line 5642
    s/ Bush\) \) \(VP \(VBD allowed\)  \(S  \(NP-SBJ \(-NONE- \*-1/ Bush\) \) \(VP \(VBD allowed\)  \(S  \(NP-SBJ \(-NONE- \*T\*-1/;
    # line 6447
    s/WHNP-1 \(WP What\) \) \(S  \(NP-SBJ \(-NONE- \*-1/WHNP-1 \(WP What\) \) \(S  \(NP-SBJ \(-NONE- \*T\*-1/;
    # line 7650
    s/-NONE- \*-3\) \) \(VP \(VBZ is\)  \(VP \(VBN caused/-NONE- \*T\*-3\) \) \(VP \(VBZ is\)  \(VP \(VBN caused/;
    # line 7725
    s/ interviewer\) \) \(NP \(-NONE- \*-1\) \) \(NP-TMP \(NN yesterday/ interviewer\) \) \(NP \(-NONE- \*T\*-1\) \) \(NP-TMP \(NN yesterday/;
    # line 8354
    ...
    # line 13344
    s/VBN syndicated\) \(NN program\) \) \(S  \(NP-SBJ-1 \(-NONE- \*-3/VBN syndicated\) \(NN program\) \) \(S  \(NP-SBJ-1 \(-NONE- \*T\*-3/;
    # line 15562
    s/VBD screamed\)  \(NP \(-NONE- \*-1\) \)\) \(NP-SBJ \(DT the/VBD screamed\)  \(NP \(-NONE- \*T\*-1\) \)\) \(NP-SBJ \(DT the/; 
    # line 32942
    s/DT that\) \(NN roster\) \) \(NP-PRD \(-NONE- \*-1/DT that\) \(NN roster\) \) \(NP-PRD \(-NONE- \*T\*-1/;
    # line 34448
    s/WDT that\) \) \(S  \(NP-SBJ \(-NONE- \*-1\) \) \(VP \(VBZ has/WDT that\) \) \(S  \(NP-SBJ \(-NONE- \*T\*-1\) \) \(VP \(VBZ has/;
    # line 37777
    s/WDT which\) \) \(S  \(NP-SBJ-2 \(-NONE- \*-1\) \) \(PP \(IN like/WDT which\) \) \(S  \(NP-SBJ-2 \(-NONE- \*T\*-1\) \) \(PP \(IN like/; 
  }
  ...
  if (!$opts{c}) {
     ...
     s/establsh/establish/;                      # line 25545
     s/syndciate/syndicate/;                     # line 21115
     s/contribued/contributed/;                  # line 32739
     s/\(VB[ND] repurchase\)/\(VB repurchase\)/; # line 18270 & ...
     s/\(VB[ND] build\)/\(VB build\)/;           # line 35473
     s/\(VBN dead\)/\(JJ dead\)/;                # line 30656
     s/\(PP \(IN by\) \(NP-LGS-3 \(NN Moody\) \(POS 's\)\)\)\) \(CC and\) \(VP \(ADJP-PRD=1 \(JJ SP1-plus\)\) \(PP \(IN by\) \(NP-LGS=3 \(NNP S&P\)\)/\(PP-3 \(IN by\) \(NP-LGS \(NN Moody\) \(POS 's\)\)\)\) \(CC and\) \(VP \(ADJP-PRD=1 \(JJ SP1-plus\)\) \(PP=3 \(IN by\) \(NP-LGS \(NNP S&P\)\)/;
     # line 1409&al:
     s/\(IN for\) (\(RB now\)|\(ADVP[^ ]* \(RB now\)\))/\(IN for\) \(NP \(RB now\)\)/;
     # line 10016:
     s/\(IN for\) \(RB good\)/\(IN for\) \(NP good\)/;
  }
...
#    # line 24513:
  ...
  while ( $_ =~ /\([^\(\)]*\)/ ) {
    ...
    # missing -NONE- for passive (e.g. 'based on')
#    s/{(PP[^ ]* <VBN [^ ]*>) (<PP.*)}/{\1 <NP <-NONE- *>> \2}/;
