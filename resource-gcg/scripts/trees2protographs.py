...
#  if len(ns)<5 and quants.none_of(t.ch,lambda et: ns in et.c):
  ...
  #### rule Rbogus2 -- parenthetical should really have been annotated something else...
    ...
    t.e = t.ch[1].e
  #### rule Ha/Hb...
  elif ('-h' not in t.c and '-h' in t.ch[0].c) or t.ch[0].c.startswith(t.c+'-h') or ('-g' not in t.c and '-g' in t.ch[0].c):  # backward gap should really have been annotated '-h'.
...
#    os += econt ( t.ch[1], i+wid(t.ch[0]), ['??'] if '-a' in t.ch[1].c else [], c, n if ns in t.ch[1].c else [] )
  ...
  #### rule Cb...
    ...
    t.e = t.ch[0].e
  #### rule TBD, merge unanalyzed proper names...
  elif t.ch[0].c=='U' and t.ch[1].c=='U':
    os += econt ( t.ch[0], i,              a, c, n if ns in t.ch[0].c else [] )
    os += econt ( t.ch[1], i+wid(t.ch[0]), a, c, n if ns in t.ch[1].c else [] )
    #os += [t.ch[0].e+'r,=,'+t.ch[1].e+'r', t.ch[0].e+'s,=,'+t.ch[1].e+'s']
    os += [t.ch[0].e+'r,r,'+t.ch[1].e+'s']
    t.e = t.ch[0].e
  #### fail...
  else:
    sys.stderr.write ( 'WARNING: unable to induce composition rule; assuming conjunction for: ' + t.c + ' ' + t.ch[0].c + ' ' + t.ch[1].c + '\n' )
    os += econt ( t.ch[0], i,              a, c, n if ns in t.ch[0].c else [] )
    os += econt ( t.ch[1], i+wid(t.ch[0]), a, c, n if ns in t.ch[1].c else [] )
    os += [t.ch[1].e+'r,r,'+t.ch[0].e+'s']
    t.e = t.ch[1].e
