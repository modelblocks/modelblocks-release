...
def tokenize(dictionary, path):
    ...
    with open(path, 'r', encoding="utf8") as f:
        ...
        for i in range(len(tokens)):
            if tokens[i] in dictionary.word2idx:
                ids[i] = dictionary.word2idx[tokens[i]]
            # hacked to accommodate parentheses in Dundee (-LRB-, -RRB-)
            elif tokens[i] == "-LRB-":
                ids[i] = dictionary.word2idx["("]
            elif tokens[i] == "-RRB-":
                ids[i] = dictionary.word2idx[")"]
            else:
                ids[i] = dictionary.word2idx["<unk>"]
