      	...
      	for (int pState=0; pState<numStates; pState++){
        	...
        	for (int np = 0; np < nParentStates; np++) {
              ...
              if (viterbi) iScore[start][end][pState][np] = Math.max(iScore[start][end][pState][np], thisCell[np]); 
								...
								iScore[start][end][pState][np] = iScore[start][end][pState][np] + thisCell[np];
								// added by exley
								System.err.printf("iScore[%d][%d][%d][%d] = %f\n",start,end,pState,np,iScore[start][end][pState][np]);
							}
          	}          	
          }
          if (true){
...
//	}
	...
	public Tree<String> extractBestViterbiParse(int gState, int gp, int start, int end, List<String> sentence ) {
		...
		if (end - start == 1) {
	      ...
	      if (goalStr1==null)
	        System.err.println("goalStr1==null with newIndex=="+newIndex+" goalStr=="+goalStr);
	      List<Tree<String>> child = new ArrayList<Tree<String>>();
