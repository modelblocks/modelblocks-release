 ...
 private:
  ...
  void update ( const X& x ) {
    ...
    if ( REPORT_COMPLEX ) {
      ...
      if ( t > 1 ) { /* ignore first timestep since no previous metrics have been computed yet */
        cerr <<"\nTotal Surprisal: "<<prevtotsurp;
        cerr <<"\nLexical Surprisal: "<<prevlexsurp;
        cerr <<"\nSyntactic Surprisal: "<<prevsynsurp;
        cerr <<"\nEntropy Reduction: "<<preventred;
        cerr <<"\nEmbedding Depth: "<<prevembeddep;
        cerr <<"\nEmbedding Difference: "<<prevembedif;
        cerr << "\n";
        meas.write();
      }
